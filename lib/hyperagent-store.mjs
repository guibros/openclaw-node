...
export class HyperAgentStore {
  ...
  #runMigrations() {
    this.#db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS ha_notify_outbox (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        item_type TEXT NOT NULL CHECK(item_type IN ('reflection','proposal')),
  ...
  putReflection({ node_id, soul_id, telemetry_from_id, telemetry_to_id, telemetry_count, raw_stats }) {
    ...
    const insertTx = this.#db.transaction(() => {
      ...
      // Creation and its operator-signal obligation are atomic (1.1): a crash
      // after commit leaves a retriable outbox row, never a silent pending
      // reflection aging toward its 24h expiry.
      this.#enqueueNotification('reflection', r.lastInsertRowid);
      return r;
    });
    const result = insertTx.immediate();
  ...
  putProposal({ reflection_id, node_id, soul_id, title, description, proposal_type, target_ref = null, diff_content = null }) {
    ...
    const insertTx = this.#db.transaction(() => {
      ...
      `).run(reflection_id, reflection.node_id, reflection.soul_id, title, description,
        proposal_type, target_ref, diff_content, domain, subdomain);
      this.#enqueueNotification('proposal', r.lastInsertRowid); // atomic with creation (1.1)
      return r;
    });
    const result = insertTx.immediate();
  ...
  #enqueueNotification(itemType, itemId) {
    this.#db.prepare(`
      INSERT OR IGNORE INTO ha_notify_outbox (item_type, item_id, event_id)
      VALUES (?, ?, ?)
    `).run(itemType, itemId, `hyperagent-${itemType}:${itemId}`);
  }

  /**
   * Deliver undelivered operator signals (1.1). deliver(evt) is awaited per
   * row; the row is marked delivered only after deliver resolves. Retries are
   * safe end-to-end: the UNIQUE event_id here plus the ledger's stable-id
   * dedup mean any crash/retry interleaving lands exactly one ledger identity.
   */
  async drainNotifyOutbox(deliver) {
    const pending = this.#db.prepare(
      'SELECT * FROM ha_notify_outbox WHERE delivered_at IS NULL ORDER BY id'
    ).all();
    let delivered = 0;
    for (const row of pending) {
      const item = row.item_type === 'reflection'
        ? this.#db.prepare('SELECT * FROM ha_reflections WHERE id = ?').get(row.item_id)
        : this.#db.prepare('SELECT * FROM ha_proposals WHERE id = ?').get(row.item_id);
      await deliver({
        id: row.event_id,
        item_type: row.item_type,
        item,
      });
      this.#db.prepare("UPDATE ha_notify_outbox SET delivered_at = datetime('now') WHERE id = ?")
        .run(row.id);
      delivered += 1;
    }
   ...
   */
  /**
   * Deterministic cohort accounting for a run_id (hyperagent-evidence 0.3).
   * Pure aggregation — no timestamps, no randomness: identical DB snapshot +
   * runId ⇒ byte-identical JSON. Coverage is COVERAGE, never effectiveness.
   */
  cohortReport(runId) {
    const rid = requiredText('runId', runId, 128);
    const all = this.#db.prepare('SELECT * FROM ha_telemetry WHERE run_id = ? ORDER BY id').all(rid);
    const cls = (r) => r.execution_class || 'unknown';
    const distinct = (rows, key) => [...new Set(rows.map((r) => r[key]).filter(Boolean))].sort();
    const countBy = (rows, keyFn) => {
      const m = {};
      for (const r of rows) { const k = keyFn(r) ?? 'null'; m[k] = (m[k] || 0) + 1; }
      return Object.fromEntries(Object.entries(m).sort(([a], [b]) => a.localeCompare(b)));
    };

    const real = all.filter((r) => cls(r) === 'real');
    const identities = {};
    for (const r of real) {
      const k = `${r.node_id}/${r.soul_id}`;
      identities[k] = identities[k] || { rows: 0, logical_tasks: new Set() };
      identities[k].rows += 1;
      identities[k].logical_tasks.add(r.logical_task_id || r.task_id || String(r.id));
    }
    ...
    return {
      ...
      totals: {
        rows: all.length,
        sessions: distinct(all, 'session_id').length,
        logical_tasks: distinct(all, 'logical_task_id').length,
      },
      by_execution_class: countBy(all, cls),
      cohort: {
        rows: real.length,
        logical_tasks: distinct(real, 'logical_task_id').length,
        sessions: distinct(real, 'session_id').length,
        by_domain: countBy(real, (r) => r.domain),
        by_mode: countBy(real, (r) => r.collaboration_mode),
        outcomes: countBy(real, (r) => r.outcome),
        by_identity: Object.fromEntries(Object.entries(identities)
          .sort(([a], [b]) => a.localeCompare(b))
          .map(([k, v]) => [k, { rows: v.rows, logical_tasks: v.logical_tasks.size, reflection_eligible: v.rows >= 5 }])),
        completeness: {
          missing_session_id: real.filter((r) => !r.session_id).length,
          missing_provider: real.filter((r) => !r.provider).length,
          missing_model: real.filter((r) => !r.model).length,
        },
        strategy_coverage: {
          rows_with_strategy: real.filter((r) => r.strategy_id != null).length,
          note: 'coverage of approved-strategy attribution — NOT effectiveness',
        },
      },
      failures: {
        natural: real.filter((r) => r.outcome === 'failure').length,
        induced: all.filter((r) => cls(r) === 'chaos' && r.outcome === 'failure').length,
      },
    };
  }
