...
export const DEFAULT_ANALYSIS_MAX_TOKENS = Number(process.env.LLM_ANALYSIS_MAX_TOKENS) || 512;
...
export function createLlmClient(opts = {}) {
  ...
  async function generate(messages, genOpts = {}) {
    ...
    const body = useNative
      ? {
          ...
          //
          // LLM_FORCE_FREE_FORM=1 disables format:json entirely. Reason: on
          // contended hardware, llama.cpp's grammar-constrained decoder can
          // stall when no valid-grammar token has high enough probability —
          // the decoder hunts for low-prob tokens, makes no apparent progress,
          // and trips Ollama's runner watchdog at 5m01s with an HTTP 500.
          // Without format:json, the model emits free-form text (usually
          // valid JSON anyway since the prompt asks for it) that the tolerant
          // parser in extraction-prompt.mjs handles.
          ...(genOpts.jsonMode && process.env.LLM_FORCE_FREE_FORM !== '1' ? { format: 'json' } : {}),
        }
      : {
          model,
          messages,
  ...
  async function generateAnalysis(messages, genOpts = {}) {
    ...
    const runFetch = async (queueAbortSignal) => {
      ...
      const timer = setTimeout(() => localController.abort(new Error('llm-client local timeout')), genOpts.waitTimeoutMs ?? 1000);
