...
export function machineBootMs() {
  try {
    // darwin: `{ sec = 1783146635, usec = ... }`. Absolute path — launchd's
    // restricted PATH (often just /usr/bin:/bin) omits /usr/sbin, which left
    // boot=null under the daemon and degraded reboot-vs-daemon-down labeling.
