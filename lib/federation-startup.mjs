...
/**
 * Default node-id resolver. Matches memory-subscriber.mjs's convention but
 * sanitizes the hostname so it satisfies the envelope schema's NODE_ID_RE
 * (F-P404 added `^[A-Za-z0-9._-]+$` to defend against DoS-via-megabyte-string).
 *
 * F-Q101 fix: macOS / iCloud auto-generates hostnames like
 * `Guillaume's-MacBook-Pro` (apostrophe + spaces) and `Gui's MBP`. Without
 * sanitization, every signed event from such machines fails envelope parse
 * at the boundary → broadcaster reports {suppressed:true, reason:'validation_error'}
 * → silent federation outage on the most common consumer-hardware target.
 */
export function defaultNodeId() {
  if (process.env.OPENCLAW_NODE_ID) return process.env.OPENCLAW_NODE_ID;
  const raw = os.hostname() || 'unknown-host';
  // Replace any disallowed run with a single dash; collapse repeats; trim.
  const sanitized = raw
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/-+/g, '-')
