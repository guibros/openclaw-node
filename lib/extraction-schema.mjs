...
// Read-before-write fields (openviking-adopt Block 3). The extractor is shown
// the known memories relevant to the transcript; `ref` names the existing
// entity a mention resolves to, `aliases` carries the spellings seen for it,
// `supersedes` names the decision a new one revises. All optional: an
// extractor that ignores them behaves exactly as before.
...
export const EntitySchema = z.object({
  ...
  ref: z.number().int().positive().nullable().optional(),
  aliases: z.array(z.string().min(1).max(FIELD_CAPS.name)).max(MAX_ALIASES_PER_ENTITY).optional(),
});
...
export const DecisionSchema = z.object({
  ...
  supersedes: z.number().int().positive().nullable().optional(),
});
