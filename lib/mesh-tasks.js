...
class TaskStore {
  ...
  async claim(nodeId) {
    ...
    for (const task of available) {
      // Hard exclusion
      if (task.exclude_nodes && task.exclude_nodes.includes(nodeId)) { excluded++; continue; }

      // Respect dependencies
      if (task.depends_on && task.depends_on.length > 0) {
        const depsReady = await this._checkDeps(task.depends_on, task.task_id);
        if (!depsReady) { depBlocked++; continue; }
      }
    ...
    console.log(`[tasks] claim: ${claimable.length} eligible for ${nodeId} (${available.length} total, excluded=${excluded}, depBlocked=${depBlocked})`);
    ...
    if (!result) {
      console.log(`[tasks] claim race: ${task.task_id} already taken`);
    }
    return result;
  }
  ...
  async findOverBudget() {
    ...
    const overBudget = [...running, ...claimed].filter(t => this.isOverBudget(t));
    console.log(`[tasks] overBudget: ${overBudget.length} tasks`);
    return overBudget;
  }
  ...
  async findStalled(stallMinutes = 5) {
    ...
    const stalled = [...running, ...claimed].filter(t => {
      const lastSignal = t.last_activity || t.started_at || t.claimed_at;
      return lastSignal && new Date(lastSignal) < cutoff;
    });
    console.log(`[tasks] stalled: ${stalled.length} tasks`);
    return stalled;
  }

  async _checkDeps(depIds, taskId) {
    const deps = await Promise.all(depIds.map(id => this.get(id)));
    const completed = deps.filter(dep => dep && dep.status === TASK_STATUS.COMPLETED).length;
    if (depIds.length > 0) {
      console.log(`[tasks] deps check for ${taskId}: ${depIds.length} deps, ${completed}/${depIds.length} complete`);
    }
...
module.exports = {
  createTask: tracer.wrap('createTask', createTask, { tier: 1, category: 'state_transition' }),
  TaskStore, TASK_STATUS, KV_BUCKET,
};
