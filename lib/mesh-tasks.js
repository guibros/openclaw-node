...
class TaskStore {
  ...
  async claim(nodeId) {
    ...
    tracer.log('info', `claim: ${claimable.length} eligible for ${nodeId} (${available.length} total, excluded=${excluded}, depBlocked=${depBlocked})`);
    ...
    if (!result) {
      tracer.log('info', `claim race: ${task.task_id} already taken`);
    }
    return result;
  }
  ...
  async findOverBudget() {
    ...
    tracer.log('info', `overBudget: ${overBudget.length} tasks`);
    return overBudget;
  }
  ...
  async findStalled(stallMinutes = 5) {
    ...
    tracer.log('info', `stalled: ${stalled.length} tasks`);
    return stalled;
  }
  ...
  async _checkDeps(depIds, taskId) {
    ...
    if (depIds.length > 0) {
      tracer.log('info', `deps check for ${taskId}: ${depIds.length} deps, ${completed}/${depIds.length} complete`);
    }
