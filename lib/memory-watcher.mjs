...
export function createAnomalyDetector(opts = {}) {
  ...
  function evaluate(record) {
    ...
    if (extracted.length >= extractionRateMinSample) {
      ...
      if (rate >= extractionRateThreshold && canFire('extraction_noop_rate')) {
        fired('extraction_noop_rate');
        alerts.push(buildAlert(
          'extraction_noop_rate',
  ...
  function evaluateStale() {
    ...
    const lastEvent = recentEvents.findLast(e => e.op?.startsWith('memory.'));
    if (!lastEvent) return [];
