...
import { mergeFields, newAliases, canonicalAlias } from './memory-types.mjs';
...
export function createExtractionStore(opts = {}) {
  ...
  // ── Aliases + decision supersession (v6) — openviking-adopt Block 3 ────
  // Typed merge (lib/memory-types.mjs): a re-mention under a new spelling is
  // an ALIAS of the existing entity (entity_aliases, keyed on the canonical
  // spelling so lookups are one indexed read), and a decision that revises an
  // earlier one SUPERSEDES it (decisions.superseded_by) instead of living
  // beside it. Both are the read-before-write step the review asked for:
  // the extractor is shown known memories and answers with ref/supersedes.
  db.exec(`
  ...
  `);
  {
    ...
    if (!cols.includes('superseded_by')) {
      db.exec('ALTER TABLE decisions ADD COLUMN superseded_by INTEGER');
    }
    db.exec('CREATE INDEX IF NOT EXISTS idx_decisions_superseded ON decisions(superseded_by)');
  }
  if (getVersion(db) < 6) setVersion(db, 6);
  ...
  `);
  const getEntityById = db.prepare(`
    SELECT id, name, type, canonical_name, first_seen, last_seen FROM entities WHERE id = ?
  `);
  // Field values come from mergeFields('entity', …): type is immutable
  // (first classification stays — F-H12), last_seen replaces.
  const updateEntity = db.prepare(`
    UPDATE entities SET last_seen = @last_seen, type = @type WHERE id = @id
  `);
  const getEntityByAlias = db.prepare(`
    SELECT entity_id AS id FROM entity_aliases WHERE alias_canonical = ?
  `);
  const getAliasesFor = db.prepare(`
    SELECT alias FROM entity_aliases WHERE entity_id = ? ORDER BY created_at, rowid
  `);
  const insertAlias = db.prepare(`
    INSERT OR IGNORE INTO entity_aliases (alias_canonical, alias, entity_id, created_at)
  ...
  // Decision identity is (session_id, decision). A re-statement in a later
  // flush merges by the registry's ops (rationale/confidence/created_at
  // replace, superseded_by immutable) — recall state (salience,
  // last_recalled, private) on the existing row is never touched.
  const getDecisionByIdentity = db.prepare(`
    SELECT id, rationale, confidence, created_at, superseded_by FROM decisions
  ...
  const updateDecision = db.prepare(`
    UPDATE decisions SET rationale = @rationale, confidence = @confidence, created_at = @created_at WHERE id = @id
  `);
  const decisionExists = db.prepare(`SELECT id FROM decisions WHERE id = ?`);
  // Only an un-superseded decision can be superseded, never by itself, and
  // never in a cycle (the target must not already supersede the new one).
  const supersedeDecision = db.prepare(`
    UPDATE decisions SET superseded_by = @by
    WHERE id = @id AND superseded_by IS NULL AND id != @by
      AND NOT EXISTS (SELECT 1 FROM decisions n WHERE n.id = @by AND n.superseded_by = @id)
  `);
  ...
  function storeExtractionResult(sessionId, result, provenance, opts = {}) {
    ...
    const doStore = db.transaction(() => {
      ...
      for (const entity of result.entities) {
        ...
        // Read-before-write resolution order: canonical name → known alias →
        // the extractor's `ref` (a KNOWN MEMORY id it was shown). A ref that
        // resolves registers this spelling as an alias of that entity.
        let row = getEntityByCanonical.get(canonical);
        let viaAlias = false;
        if (!row) {
          const a = getEntityByAlias.get(canonical);
          if (a) { row = { id: a.id }; viaAlias = true; }
        }
        if (!row && Number.isInteger(entity.ref)) {
          const target = getEntityById.get(entity.ref);
          if (target) {
            row = { id: target.id };
            viaAlias = true;
            insertAlias.run({ alias_canonical: canonical, alias: entity.name, entity_id: target.id, created_at: now });
            stats.aliases_added++;
          }
        }
        if (row) {
          const existing = getEntityById.get(row.id);
          const { merged } = mergeFields('entity', existing, { name: entity.name, type: entity.type, last_seen: now });
          updateEntity.run({ id: row.id, type: merged.type, last_seen: merged.last_seen });
          stats.entities_matched++;
          if (viaAlias) stats.aliases_resolved++;
        } else {
          stats.entities_new++;
        ...
        // Spellings the extractor saw for this entity (aliases: UNION). An
        // alias never steals another entity's identity: a candidate that is
        // already some entity's canonical name or alias is skipped.
        if (row && Array.isArray(entity.aliases) && entity.aliases.length) {
          const existing = getEntityById.get(row.id);
          const have = getAliasesFor.all(row.id).map(r => r.alias);
          for (const alias of newAliases(existing.name, have, entity.aliases)) {
            const c = canonicalAlias(alias);
            if (getEntityByCanonical.get(c) || getEntityByAlias.get(c)) continue;
            insertAlias.run({ alias_canonical: c, alias, entity_id: row.id, created_at: now });
            stats.aliases_added++;
          }
        }
      ...
      // Store decisions (merge by registry ops; supersede on request)
      for (const decision of result.decisions) {
        const existing = getDecisionByIdentity.get(sessionId, decision.decision);
        let id;
        if (existing) {
          const { merged } = mergeFields('decision', existing, {
            rationale: decision.rationale,
            confidence: decision.confidence,
            created_at: now,
          });
          updateDecision.run({ id: existing.id, rationale: merged.rationale, confidence: merged.confidence, created_at: merged.created_at });
          id = existing.id;
        ...
        if (Number.isInteger(decision.supersedes) && decision.supersedes !== id && decisionExists.get(decision.supersedes)) {
          const r = supersedeDecision.run({ id: decision.supersedes, by: id });
          if (r.changes) stats.decisions_superseded++;
        }
      }
  ...
  // ── Read-before-write: known-memory candidates ───────────────────
  //
  // What the extractor is shown before it writes. Entities whose canonical
  // name or alias occurs in the transcript text, plus decisions whose FTS
  // terms overlap it. Bounded and cheap: one indexed read per table, string
  // scans over at most `maxScan` names.
  const listEntitiesForScan = db.prepare(`
    SELECT id, name, type, canonical_name FROM entities
    ORDER BY mention_count DESC, last_seen DESC LIMIT ?
  `);
  const listAliasesForScan = db.prepare(`
    SELECT a.alias_canonical, a.alias, a.entity_id FROM entity_aliases a LIMIT ?
  `);
   ...
   * @param {{maxEntities?:number, maxDecisions?:number, maxScan?:number}} [opts]
   * @returns {{entities: Array<{id:number,name:string,type:string,aliases:string[]}>, decisions: Array<{id:number,decision:string}>}}
   */
  function findCandidateMemories(text, opts = {}) {
    const maxEntities = opts.maxEntities ?? 40;
    const maxDecisions = opts.maxDecisions ?? 10;
    const maxScan = opts.maxScan ?? 2000;
    const hay = canonicalAlias(text);
    if (!hay) return { entities: [], decisions: [] };
    // Whole words of the transcript, for partial-name matches: "JetStream"
    // in the text should surface the entity "NATS JetStream". A name matches
    // when its full canonical form occurs, or any of its tokens of ≥5 chars
    // occurs as a whole word. Over-matching is cheap here (this is a hint
    // list for the extractor, capped and ordered by mention_count), missing
    // a known entity is what mints duplicates.
    const words = new Set(hay.split(/[^a-z0-9]+/).filter(Boolean));
    const mentions = (canonical) => {
      if (canonical.length >= 3 && hay.includes(canonical)) return true;
      return canonical.split(' ').some((t) => t.length >= 5 && words.has(t));
    };

    const aliasesByEntity = new Map();
    const hitIds = new Set();
    for (const a of listAliasesForScan.all(maxScan)) {
      if (!aliasesByEntity.has(a.entity_id)) aliasesByEntity.set(a.entity_id, []);
      aliasesByEntity.get(a.entity_id).push(a.alias);
      if (mentions(a.alias_canonical)) hitIds.add(a.entity_id);
    }
    const entities = [];
    for (const e of listEntitiesForScan.all(maxScan)) {
      const c = e.canonical_name || canonicalizeName(e.name);
      if (mentions(c) || hitIds.has(e.id)) {
        entities.push({ id: e.id, name: e.name, type: e.type, aliases: aliasesByEntity.get(e.id) || [] });
        if (entities.length >= maxEntities) break;
      }
    }

    let decisions = [];
    const terms = [...new Set(hay.split(/[^a-z0-9]+/).filter(t => t.length >= 5))].slice(0, 24);
    if (terms.length) {
      try {
        decisions = db.prepare(`
          SELECT d.id, d.decision
          FROM (SELECT rowid, bm25(decisions_fts) AS score FROM decisions_fts WHERE decisions_fts MATCH ? ORDER BY score LIMIT ?) m
          JOIN decisions d ON d.id = m.rowid
          WHERE d.superseded_by IS NULL
        `).all(terms.map(t => `"${t}"`).join(' OR '), maxDecisions * 2);
      } catch { decisions = []; }
    }
    if (decisions.length < maxDecisions) {
      const seen = new Set(decisions.map(d => d.id));
      for (const d of recentDecisions.all(maxDecisions)) {
        if (!seen.has(d.id)) decisions.push(d);
        if (decisions.length >= maxDecisions) break;
      }
    }
    return { entities, decisions: decisions.slice(0, maxDecisions) };
  }

  function getEntityAliases(entityId) {
    return getAliasesFor.all(entityId).map(r => r.alias);
  }
  ...
  function generateMemoryContent(charBudget = 2200) {
    ...
    // Recent Decisions — last 5 still in force, high confidence first
    const recentDecisions = db.prepare(`
