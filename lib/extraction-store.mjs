...
export function createExtractionStore(opts = {}) {
  ...
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_mentions_dedup
      ON mentions(session_id, entity_id, IFNULL(turn_index, -1));
  ...
  const upsertEntity = db.prepare(`
    ...
    ON CONFLICT(name) DO UPDATE SET
      last_seen = @now,
      -- mention_count is derived from the mentions table after each flush
      -- (recomputeMentionCount), not bumped per upsert — a re-extracted
      -- overlapping tail must not inflate it (D5).
  ...
  // OR IGNORE: a re-extracted tail re-presents the same (session, entity, turn)
  // mention — keep the first, don't duplicate (idx_mentions_dedup).
  const insertMention = db.prepare(`
    INSERT OR IGNORE INTO mentions (entity_id, session_id, turn_index, salience, created_at, source_type, source_node, source_event_id)
