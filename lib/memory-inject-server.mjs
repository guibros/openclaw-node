...
async function resolveDeps(deps = {}) {
  ...
  // The open races Phase-0's wholesale re-import at daemon boot (SQLITE_IOERR
  // during the heavy WAL transaction) and used to give up PERMANENTLY — the whole
  // boot then ran with the privacy filter rejecting everything. Retry a few times
  // before conceding (the daemon's own extraction-store init recovers ~1min in).
  if (!extractionDb) {
    const { createExtractionStore } = await import('./extraction-store.mjs');
    let lastErr;
    for (let attempt = 1; attempt <= 5 && !extractionDb; attempt++) {
      try {
        extractionDb = createExtractionStore().db;
        if (attempt > 1) process.stderr.write(`[memory-inject-server] extractionDb open succeeded on attempt ${attempt}\n`);
      } catch (err) {
        lastErr = err;
    ...
    if (!extractionDb) {
      ...
      process.stderr.write(`[memory-inject-server] extractionDb open failed after 5 attempts (privacy filter will reject all results, concepts/decisions queries will be empty): ${lastErr?.message}\n`);
    }
  }
