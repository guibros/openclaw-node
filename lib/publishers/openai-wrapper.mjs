...
export function wrapOpenAI(client, publisher, opts = {}) {
  ...
  // F-P301 fix: REMOVED process-wide `memoryDisabledForSession` closure var.
  // Previously, a single `@memory none` from any user permanently disabled
  // memory for ALL subsequent calls on this client instance (the normal
  // server-side pattern is one client across many users — cross-tenant
  // leak). F-H21 specifies `none` is per-turn only; the wrapper now matches.
  // Operators that want a real session-disable should pass opts.sessionKeyFn
  // to derive a per-conversation key (TODO: future).
    ...
    if (injector && args[0]?.messages) {
      try {
        ...
        if (prompt) {
          ...
          // F-P301: 'none' is now equivalent to 'off' (per-turn skip), no
          // cross-call state. 'off' and 'none' both skip injection for THIS
          // call only.
          const skipInjection = directive.type === 'off' || directive.type === 'none';
          if (!skipInjection) {
            // F-P302 fix (F-N70 regression): unify @memory only:X semantics
            // with memory-inject-server. Server uses cleanedText as query
            // + themeFilter for post-retrieval filter. Old wrapper code
            // used the bare theme as the query (semantic search on one
            // token), giving different memory blocks for the same prompt
            // depending on frontend. Now both paths produce identical
            // injection.
