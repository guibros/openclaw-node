...
export class SessionStore {
  ...
  async importSession(jsonlPath, opts = {}) {
    ...
    // Check existing session — append-delta if it grew
    const existing = this.#db.prepare('SELECT message_count FROM sessions WHERE id = ?').get(sessionId);
    const existingCount = existing ? existing.message_count : 0;
    ...
    return { sessionId, messageCount: newMessages.length, imported: true };
  }
