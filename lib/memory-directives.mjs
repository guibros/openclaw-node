 ...
 *   @memory none   — disable injection for current turn (currently behaves as
 *                    `off` — F-H21: per-session persistence requires session
 *                    state in the HTTP inject server, which is intentionally
 *                    stateless. Use it as a per-turn opt-out for now; for
 *                    durable opt-out, set INJECTION_LOG_DISABLED=1 or
 *                    OPENCLAW_MEMORY_INJECT=off on companion-bridge)
 *   @memory only:<theme> — constrain injection to sessions matching a theme
 *                          label (LIKE-matched, case-insensitive)
 *
