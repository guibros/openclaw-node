...
export function sampleLayers(opts = {}) {
  ...
  // `!== undefined` (not ??): an EXPLICIT null means "no extraction exists" — with
  // ?? it fell through to the real state.db, so the test that injected null started
  // failing the moment live extraction was healed (it only ever passed while the
  // brain was actually frozen).
  const lastExtraction = opts.lastExtraction !== undefined ? opts.lastExtraction : readLastExtraction(stateDbPath);
