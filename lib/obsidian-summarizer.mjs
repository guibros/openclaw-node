...
export function buildConceptFrontmatter(entity, relatedEntities = [], avgSalience, opts = {}) {
  const lines = [
    ...
    // the alias is what makes [[Entity Name]] resolve in Obsidian.
    `aliases: ["${entity.name.replace(/"/g, '\\"')}"]`,
  ...
  if (resolvable.length > 0) {
    const wikilinks = resolvable.map(n => `[[${n}]]`);
...
export function buildConceptBody(entityName, opts = {}) {
  ...
  if (recentSessions.length > 0) {
    ...
    // otherwise plain text — a raw [[sessions/<uuid>]] never matched the
    // date-topic-shortid note filenames and was always dangling.
    ...
    const sessLines = recentSessions.map(s => {
      ...
      return note ? `- [[sessions/${note}]]` : `- session ${s.session_id}`;
    });
...
export async function generateConceptNotes(opts) {
  ...
  try {
    ...
    for (const data of conceptData) {
      ...
      const frontmatter = buildConceptFrontmatter(entity, relatedEntities, avgSalience, { resolvableSlugs });
      const summary = await generateConceptSummary(client, entity.name, mentions, { signal });
      const body = buildConceptBody(entity.name, { summary, decisions, recentSessions, sessionNoteResolver });
