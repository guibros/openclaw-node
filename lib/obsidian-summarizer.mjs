  ...
  try {
    if (typeof client.generateAnalysis === 'function') {
      const result = await client.generateAnalysis(messages, {
        maxTokens: 256,
        temperature: 0.3,
        waitTimeoutMs: opts.waitTimeoutMs ?? 3000,
      });
 ...
 * @param {{ respectPrivacy?: boolean }} [opts] — F-N102 fix: filter private
 *   entities/decisions from vault generation. Defaults to true. The vault is
 *   a user-facing surface that's typically synced (iCloud/Dropbox); private
 *   memory must NOT land there.
 * @returns {Array<object>} concept data objects
 */
export function queryConceptData(db, threshold, opts = {}) {
  // F-N102: privacy on by default. To dump everything to a local-only debug
  // vault you must explicitly opt out via { respectPrivacy: false }.
  const respectPrivacy = opts.respectPrivacy !== false;
 ...
 * @param {boolean} [opts.respectPrivacy=true] — F-N102 fix. When true (default),
 *   private entities + decisions are excluded from vault generation. The
 *   vault is typically iCloud/Dropbox-synced; private content must not land
 *   there. Set to false only for a local-only "audit my own private memory"
 *   vault that you know isn't synced.
...
export async function generateConceptNotes(opts) {
  ...
  try {
    ...
    const allCandidates = queryConceptData(db, threshold, { respectPrivacy });
