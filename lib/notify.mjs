...
// The branded bundle (build-notifier-app.sh) is preferred over the stock brew
// binary: the sender's bundle icon is what macOS shows on the LEFT of the
// banner, and only our own bundle can carry the OpenClaw icon there.
export function findTerminalNotifier(env = process.env) {
  const home = env.OPENCLAW_NOTIFY_HOME || os.homedir();
  const candidates = [
    env.OPENCLAW_NOTIFIER_APP,
    path.join(home, '.openclaw', 'share', 'OpenClawNotifier.app', 'Contents', 'MacOS', 'terminal-notifier'),
...
export async function notify(input, opts = {}) {
  ...
  // Stable-id dedup (hyperagent-evidence 1.1): a caller-supplied id makes the
  // event idempotent — if the ledger already carries it, neither dispatch nor
  // append happens. Outbox-style retries land exactly one ledger identity.
  if (input.id) {
    try {
      const existing = fs.readFileSync(paths.ledger, 'utf-8');
      if (existing.includes(`"id":"${input.id}"`)) {
        return { v: 1, id: input.id, deduped: true };
      }
    } catch { /* no ledger yet — first event proceeds */ }
  }
  const evt = {
    v: 1,
    type: 'notification',
