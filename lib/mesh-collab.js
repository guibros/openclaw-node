...
class CollabStore {
  ...
  async _updateWithCAS(key, mutateFn, maxRetries = 3) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      try {
        ...
        console.warn(`[CAS] conflict on ${key}, attempt ${attempt + 1}/${maxRetries}`);
        // CAS conflict — retry
      }
    }
  }
  ...
  async addNode(sessionId, nodeId, role = 'worker', scope = '*') {
    return this._updateWithCAS(sessionId, (session) => {
      if (session.status !== COLLAB_STATUS.RECRUITING) {
        console.log(`[collab] addNode rejected: ${sessionId} reason=session not recruiting (status=${session.status})`);
        return null;
      }

      // Check max_nodes
      if (session.max_nodes && session.nodes.length >= session.max_nodes) {
        console.log(`[collab] addNode rejected: ${sessionId} reason=max_nodes reached (${session.nodes.length}/${session.max_nodes})`);
        return null;
      }

      // Check duplicate
      if (session.nodes.find(n => n.node_id === nodeId)) {
        console.log(`[collab] addNode rejected: ${sessionId} reason=duplicate node ${nodeId}`);
        return null;
      }
      ...
      console.log(`[collab] addNode: ${nodeId} → session ${sessionId} (${session.nodes.length} nodes)`);
      return session;
    });
  }
  ...
  async removeNode(sessionId, nodeId) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      console.log(`[collab] removeNode: ${nodeId} from ${sessionId}`);
      return session;
    });
  }
  ...
  async setNodeStatus(sessionId, nodeId, status) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      console.log(`[collab] nodeStatus: ${nodeId} → ${status} in ${sessionId}`);
      return session;
    });
  }
  ...
  async startRound(sessionId) {
    ...
    const result = await this._updateWithCAS(sessionId, (session) => {
      ...
      if (deadNodes.length > 0) {
        ...
        console.log(`[collab] startRound: pruned ${deadNodes.length} dead nodes from ${sessionId}`);
      }

      // Check if we still have enough nodes after pruning
      if (session.nodes.length < session.min_nodes) {
        console.warn(`[collab] startRound ABORT: ${sessionId} only ${session.nodes.length} nodes (min: ${session.min_nodes})`);
      ...
      console.log(`[collab] startRound: R${session.current_round} for ${sessionId} (${session.nodes.length} nodes)`);
      return session;
    });
  ...
  async submitReflection(sessionId, reflection) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      if (session.status !== COLLAB_STATUS.ACTIVE) {
        console.log(`[collab] submitReflection rejected: ${sessionId} reason=session not active (status=${session.status})`);
        return null;
      }

      const currentRound = session.rounds[session.rounds.length - 1];
      if (!currentRound) {
        console.log(`[collab] submitReflection rejected: ${sessionId} reason=no current round`);
        return null;
      }

      // Prevent duplicate reflections from same node
      if (currentRound.reflections.find(r => r.node_id === reflection.node_id)) {
        console.log(`[collab] submitReflection rejected: ${sessionId} reason=duplicate from ${reflection.node_id}`);
        return null;
      }
      ...
      console.log(`[collab] submitReflection: ${reflection.node_id} R${currentRound.round_number} vote=${reflection.vote || 'continue'}`);
      return session;
    });
  }
  ...
  checkConvergence(session) {
    ...
    console.log(`[collab] convergence: ${converged ? 'YES' : 'NO'} session=${session.session_id} valid=${validReflections.length} quorum=${minQuorum} parseFailures=${parseFailures.length}`);
    return converged;
  }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        if (attempt === 2) {
          ...
          } catch (recoveryErr) { console.warn(`[mesh-collab] artifact recovery write failed for ${sessionId}/${key}: ${recoveryErr.message}`); }
          return null;
        }
        // conflict — retry
      }
  ...
  compileDirectedInput(session, nodeId, taskDescription) {
    ...
    console.log(`[collab] directedInput: ${nodeId} phase=${phase} step=${current_step} ${result.length} chars`);
    return result;
  }
  ...
  async advanceCirclingStep(sessionId) {
    ...
    const result = await this._updateWithCAS(sessionId, (session) => {
      ...
      console.log(`[collab] circlingStep: ${oldPhase}/${oldStep} → ${c.phase}/${c.current_step} session=${sessionId}`);
  ...
  async markConverged(sessionId) {
    return this._updateWithCAS(sessionId, (session) => {
      if (['completed', 'aborted'].includes(session.status)) {
        console.log(`[collab] markConverged: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      ...
      console.log(`[collab] CONVERGED: ${sessionId} round=${session.current_round}`);
      return session;
    });
  }
  ...
  async markCompleted(sessionId, result) {
    return this._updateWithCAS(sessionId, (session) => {
      if (['completed', 'aborted'].includes(session.status)) {
        console.log(`[collab] markCompleted: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      ...
      console.log(`[collab] COMPLETED: ${sessionId}`);
      return session;
    });
  }
  ...
  async markAborted(sessionId, reason) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      if (['completed', 'aborted'].includes(session.status)) {
        console.log(`[collab] markAborted: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      session.status = COLLAB_STATUS.ABORTED;
      session.completed_at = new Date().toISOString();
      session.result = { success: false, summary: reason, aborted: true };
      console.log(`[collab] ABORTED: ${sessionId} reason=${reason}`);
      return session;
    });
  }
