...
class CollabStore {
  constructor(kv) {
    ...
    tracer.wrapClass(this, [
      ...
      'appendAudit', 'advanceCirclingStep', 'recordArtifactFailure'
  ...
  async _updateWithCAS(key, mutateFn, maxRetries = 3) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      if (!updated) {
        tracer.emit('_updateWithCAS', { tier: 2, category: 'lifecycle', args_summary: `rejected: ${key}` });
        return null;
      }
      try {
        ...
        tracer.log('warn', `CAS conflict on ${key}, attempt ${attempt + 1}/${maxRetries}`);
        // CAS conflict — retry
      }
    }
  }
  ...
  async appendAudit(sessionId, event, detail = {}) {
    try {
      ...
      if (count <= AUDIT_ERROR_LOG_LIMIT) {
        tracer.log('error', `audit append failed for ${sessionId}/${event}: ${err.message}${count === AUDIT_ERROR_LOG_LIMIT ? ' (suppressing further audit errors for this session)' : ''}`);
      }
    }
  }
  ...
  async addNode(sessionId, nodeId, role = 'worker', scope = '*') {
    return this._updateWithCAS(sessionId, (session) => {
      if (session.status !== COLLAB_STATUS.RECRUITING) {
        tracer.log('info', `addNode rejected: ${sessionId} reason=session not recruiting (status=${session.status})`);
        return null;
      }

      // Check max_nodes
      if (session.max_nodes && session.nodes.length >= session.max_nodes) {
        tracer.log('info', `addNode rejected: ${sessionId} reason=max_nodes reached (${session.nodes.length}/${session.max_nodes})`);
        return null;
      }

      // Check duplicate
      if (session.nodes.find(n => n.node_id === nodeId)) {
        tracer.log('info', `addNode rejected: ${sessionId} reason=duplicate node ${nodeId}`);
        return null;
      }
      ...
      tracer.log('info', `addNode: ${nodeId} → session ${sessionId} (${session.nodes.length} nodes)`);
      return session;
    });
  }
  ...
  async removeNode(sessionId, nodeId) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      tracer.log('info', `removeNode: ${nodeId} from ${sessionId}`);
      return session;
    });
  }
  ...
  async setNodeStatus(sessionId, nodeId, status) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      tracer.log('info', `nodeStatus: ${nodeId} → ${status} in ${sessionId}`);
      return session;
    });
  }
  ...
  async startRound(sessionId) {
    ...
    const result = await this._updateWithCAS(sessionId, (session) => {
      ...
      if (deadNodes.length > 0) {
        ...
        tracer.log('info', `startRound: pruned ${deadNodes.length} dead nodes from ${sessionId}`);
      }

      // Check if we still have enough nodes after pruning
      if (session.nodes.length < session.min_nodes) {
        tracer.log('warn', `startRound ABORT: ${sessionId} only ${session.nodes.length} nodes (min: ${session.min_nodes})`);
      ...
      tracer.log('info', `startRound: R${session.current_round} for ${sessionId} (${session.nodes.length} nodes)`);
      return session;
    });
  ...
  async submitReflection(sessionId, reflection) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      if (session.status !== COLLAB_STATUS.ACTIVE) {
        tracer.log('info', `submitReflection rejected: ${sessionId} reason=session not active (status=${session.status})`);
        return null;
      }

      const currentRound = session.rounds[session.rounds.length - 1];
      if (!currentRound) {
        tracer.log('info', `submitReflection rejected: ${sessionId} reason=no current round`);
        return null;
      }

      // Prevent duplicate reflections from same node
      if (currentRound.reflections.find(r => r.node_id === reflection.node_id)) {
        tracer.log('info', `submitReflection rejected: ${sessionId} reason=duplicate from ${reflection.node_id}`);
        return null;
      }
      ...
      tracer.log('info', `submitReflection: ${reflection.node_id} R${currentRound.round_number} vote=${reflection.vote || 'continue'}`);
      return session;
    });
  }
  ...
  checkConvergence(session) {
    ...
    switch (session.convergence.type) {
      case CONVERGENCE.UNANIMOUS:
        ...
        if (parseFailures.length > 0) {
          ...
          converged = convergedCount === reflections.length && reflections.length > 0;
        }
        break;
      ...
      case CONVERGENCE.MAJORITY: {
        ...
        converged = (convergedCount / validReflections.length) >= session.convergence.threshold;
        break;
      }
    ...
    tracer.log('info', `convergence: ${converged ? 'YES' : 'NO'} session=${session.session_id} valid=${validReflections.length} quorum=${minQuorum} parseFailures=${parseFailures.length}`);
    return converged;
  }
  ...
  async storeArtifact(sessionId, key, content) {
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (blobSize > 950_000) {
        tracer.log('error', `CRITICAL: session ${sessionId} blob is ${(blobSize / 1024).toFixed(0)}KB — approaching JetStream KV 1MB limit`);
      } else if (blobSize > 800_000) {
        tracer.log('warn', `WARNING: session ${sessionId} blob is ${(blobSize / 1024).toFixed(0)}KB — consider external artifact store`);
      }
      ...
      try {
        ...
        if (attempt === 2) {
          ...
          tracer.log('error', `storeArtifact FAILED for ${sessionId}/${key}: ${err.message}. Removing artifact and persisting without it.`);
          ...
          } catch (recoveryErr) { tracer.log('warn', `artifact recovery write failed for ${sessionId}/${key}: ${recoveryErr.message}`); }
          return null;
        }
        // conflict — retry
      }
  ...
  compileDirectedInput(session, nodeId, taskDescription) {
    ...
    tracer.log('info', `directedInput: ${nodeId} phase=${phase} step=${current_step} ${result.length} chars`);
    return result;
  }
  ...
  async advanceCirclingStep(sessionId) {
    ...
    const result = await this._updateWithCAS(sessionId, (session) => {
      ...
      tracer.log('info', `circlingStep: ${oldPhase}/${oldStep} → ${c.phase}/${c.current_step} session=${sessionId}`);
  ...
  async markConverged(sessionId) {
    return this._updateWithCAS(sessionId, (session) => {
      if (['completed', 'aborted'].includes(session.status)) {
        tracer.log('info', `markConverged: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      ...
      tracer.log('info', `CONVERGED: ${sessionId} round=${session.current_round}`);
      return session;
    });
  }
  ...
  async markCompleted(sessionId, result) {
    return this._updateWithCAS(sessionId, (session) => {
      if (['completed', 'aborted'].includes(session.status)) {
        tracer.log('info', `markCompleted: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      ...
      tracer.log('info', `COMPLETED: ${sessionId}`);
      return session;
    });
  }
  ...
  async markAborted(sessionId, reason) {
    return this._updateWithCAS(sessionId, (session) => {
      ...
      if (['completed', 'aborted'].includes(session.status)) {
        tracer.log('info', `markAborted: ${sessionId} already terminal (status=${session.status})`);
        return null;
      }
      session.status = COLLAB_STATUS.ABORTED;
      session.completed_at = new Date().toISOString();
      session.result = { success: false, summary: reason, aborted: true };
      tracer.log('info', `ABORTED: ${sessionId} reason=${reason}`);
      return session;
    });
  }
...
module.exports = {
  createSession: tracer.wrap('createSession', createSession, { tier: 1, category: 'state_transition' }),
  CollabStore,
  COLLAB_STATUS,
  COLLAB_MODE,
  CONVERGENCE,
