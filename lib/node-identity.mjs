...
export function createIdentityRegistry(opts = {}) {
   ...
   * { role: 'lead' | 'worker' } is the bus permission tier the NATS users
   * renderer gives this key (Phase 7); omitted = worker.
   */
  function trust(nodeId, pubkey, addedBy = 'tofu', opts = {}) {
    ...
    entries.set(nodeId, { pubkey, addedAt: new Date().toISOString(), addedBy, ...role });
    save();
    return true;
  }
