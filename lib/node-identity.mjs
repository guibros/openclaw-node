...
export function createIdentityRegistry(opts = {}) {
  ...
  function save() {
    ...
    try {
      ...
      // F-P411 fix: atomic write. Previously fs.writeFileSync was non-atomic;
      // a SIGKILL or disk-full mid-write left truncated JSON, and the next
      // startup's catch at the read path silently "started fresh" — ALL
      // trust bindings wiped, opening F-N3 (TOFU spoofing) for every peer
      // simultaneously. tmp + rename gives us atomic replacement.
      const tmp = `${registryPath}.tmp`;
      fs.writeFileSync(tmp, JSON.stringify(obj, null, 2), { mode: 0o600 });
      fs.renameSync(tmp, registryPath);
    } catch {
      // Best-effort — registry is recoverable from TOFU on next run.
      // Stale .tmp file cleanup is handled on next save (overwrite).
    }
  }
...
export function signEvent(event, privateKey, opts = {}) {
  if (!opts.force && (event.signature || event.signer_pubkey)) {
    throw new Error(
      ...
      'Pass { force: true } if you really want to re-sign (e.g. after explicit field cleanup).'
    );
  }
  const publicKey = crypto.createPublicKey(privateKey);
  const publicKeyBase64 = publicKey
