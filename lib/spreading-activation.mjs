 ...
 *
 * F-P201 fix: the real `createGraphCache.queryNeighbors` returns
 * `{ edges, neighbors }` (verified in bin/obsidian-graph-cache.mjs:177).
 * The previous adapter called `.map()` directly on that object, throwing
 * `TypeError: rows.map is not a function` on every call. Channel 5 (spreading
 * activation) was therefore silently disabled in production any time a real
 * graphCache instance was wired in — same silent-disable pattern as F-C14.
 * Tests passed only because the mock returned an array.
 *
 * Accepts both shapes for backward compatibility with test mocks:
 *   - { edges, neighbors } (real graphCache, post-F-P201)
 *   - Array<row> (old test mocks; still works)
 *
 * @param {{ queryNeighbors: (nodeId: string, opts: {direction: string}) => {edges: Array, neighbors: Array} | Array<{target_id: string, weight: number}> }} graphCache
 * @returns {{ edgesFrom: (nodeId: string) => Array<{target: string, weight: number}> }}
 */
export function createGraphAdapter(graphCache) {
  return {
    edgesFrom(nodeId) {
      // 'both': vault links are citations, not arrows — activation must flow
      // session-note→concept edges in reverse too, or concept seeds reach
      // nothing (channel 5 returned [] for every query while the walk was
      // outgoing-only; memory review 2026-07-04 §3B).
      const result = graphCache.queryNeighbors(nodeId, { direction: 'both' });
      const rows = Array.isArray(result)
        ? result                       // legacy/mock shape
        : (result?.edges || []);       // real graphCache shape
      const out = [];
      const seen = new Set();
      for (const r of rows) {
        const src = r.source_id ?? r.source;
        const tgt = r.target_id ?? r.target;
        // the OTHER end, whichever side nodeId is on
        const other = src === nodeId ? tgt : (tgt === nodeId ? src : (tgt ?? src));
        if (!other || other === nodeId || seen.has(other)) continue;
        seen.add(other);
        out.push({ target: other, weight: r.weight ?? 1 });
      }
      return out;
    },
  };
