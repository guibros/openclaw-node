...
export function createGraphAdapter(graphCache) {
  return {
    edgesFrom(nodeId) {
      const result = graphCache.queryNeighbors(nodeId, { direction: 'outgoing' });
      const rows = Array.isArray(result)
        ? result                       // legacy/mock shape
        : (result?.edges || []);       // real graphCache shape
      return rows.map(r => ({
        // Outgoing edges from nodeId: target is the OTHER end.
        // If source_id === nodeId, target is target_id; else target is source_id.
        // The 'outgoing' query in graph-cache filters source_id = nodeId, so
        // target_id is always the destination — keep that as the primary path.
