...
function findRole(roleId, searchDirs) {
    ...
    for (const ext of ['.yaml', '.yml']) {
      ...
      if (fs.existsSync(candidate)) {
        try {
          ...
          console.log(`[role-loader] Found role ${roleId} at ${candidate}`);
          return role;
        } catch (err) {
  ...
  console.log(`[role-loader] Role not found: ${roleId}`);
  return null;
}
...
function listRoles(searchDirs) {
  ...
  for (const dir of searchDirs) {
    ...
    for (const file of files) {
      ...
      } catch (err) { console.warn(`[role-loader] skipping malformed role ${file}: ${err.message}`); }
    }
  }
...
function validateRole(role) {
  ...
  console.log(`[role-loader] validateRole: ${errors.length === 0 ? 'PASS' : errors.length + ' issues'}`);
    ...
    if (req.type === 'file_match') {
      ...
      if (matchingFiles.length > 0 && worktreePath) {
        ...
        for (const file of matchingFiles) {
          ...
          } catch (err) { console.warn(`[role-loader] content check read failed for ${file}: ${err.message}`); }
        }
        if (!found) {
          failures.push({
            type: 'content_check',
...
function checkForbiddenPatterns(role, outputFiles, worktreePath) {
  ...
  for (const fp of role.forbidden_patterns) {
    ...
    for (const file of scopeFiles) {
      ...
      } catch (err) { console.warn(`[role-loader] forbidden pattern check failed for ${file}: ${err.message}`); }
    }
  }
