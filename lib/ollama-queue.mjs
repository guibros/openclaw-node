...
const RETRY_DELAYS   = [1000, 2000, 4000];                   // ms — exponential backoff for transients
...
function isTransient(err) {
  ...
  if (/fetch failed|socket hang up|terminated/i.test(err.message)) return true;
  // HTTP 5xx — could parse from message if our llm-client surfaces it
  if (/HTTP 50[0-9]|503|504/i.test(err.message)) return true;
  // Schema validation, 4xx, malformed JSON — persistent, don't retry
  return false;
}
