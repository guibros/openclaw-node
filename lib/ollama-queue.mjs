...
async function executeJob(type, run, opts = {}) {
  ...
  // the slot).
  const myJob = { type, started_at: startedAt, payload_size: payloadSize, eta_ms: eta, model: opts.model, abandoned: false };
...
export async function requestAnalysis(run, opts = {}) {
  ...
  const optsWithSignal = { ...opts, abortSignal: queueController.signal };
  ...
  try {
    ...
    if (winner.timeout) {
      ...
      // F-N103 fix: even after abort(), the in-flight fetch may still be
      // running if the underlying client (Ollama is the prime example)
      // doesn't honor the abort signal mid-stream. To prevent the slot
      // from staying blocked, mark the job abandoned and release the
      // slot defensively. The original executeJob's finally is now
      // idempotent (it checks `if (state.currentJob === myJob)` before
      // clearing) so it won't stomp a fresh job that takes this slot.
      if (state.currentJob) {
