...
const tracer = createTracer('mcp-knowledge-core');
...
export const scanMarkdownFiles = tracer.wrap('scanMarkdownFiles', function scanMarkdownFiles(root, includePaths, excludePatterns) {
  const files = [];
...
export const chunkMarkdown = tracer.wrap('chunkMarkdown', function chunkMarkdown(text, filePath) {
...
export const splitOversized = tracer.wrap('splitOversized', function splitOversized(section, body, level, chunks) {
...
export const hashContent = tracer.wrap('hashContent', function hashContent(text) {
  return createHash('sha256').update(text).digest('hex');
}, { tier: 3, category: 'compute' });

// ─── SQLite + sqlite-vec Storage ─────────────────────────────────────────────

export const initDatabase = tracer.wrap('initDatabase', function initDatabase(dbPath) {
  ...
  return db;
}, { tier: 1, category: 'lifecycle' });

// ─── Embedding Pipeline ──────────────────────────────────────────────────────

let embedder = null;

export const getEmbedder = tracer.wrapAsync('getEmbedder', async function getEmbedder() {
  if (!embedder) {
    try {
...
export const embed = tracer.wrapAsync('embed', async function embed(text) {
  const model = await getEmbedder();
  const result = await model(text, { pooling: 'mean', normalize: true, truncation: true, max_length: 256 });
  return new Float32Array(result.data);
}, { tier: 3, category: 'compute' });

export const embedBatch = tracer.wrapAsync('embedBatch', async function embedBatch(texts) {
  const results = [];
  for (const text of texts) {
    results.push(await embed(text));
  }
  return results;
}, { tier: 2, category: 'compute' });

// ─── Indexer ─────────────────────────────────────────────────────────────────

export const indexWorkspace = tracer.wrapAsync('indexWorkspace', async function indexWorkspace(db, root, opts = {}) {
...
export const semanticSearch = tracer.wrapAsync('semanticSearch', async function semanticSearch(db, query, limit = 10) {
...
export const findRelated = tracer.wrapAsync('findRelated', async function findRelated(db, docPath, limit = 10) {
  const safeLimit = Math.max(1, Math.min(100, Math.floor(Number(limit) || 10)));
...
export const getStats = tracer.wrap('getStats', function getStats(db) {
...
export const startPolling = tracer.wrap('startPolling', function startPolling(db, root) {
...
export const createKnowledgeEngine = tracer.wrapAsync('createKnowledgeEngine', async function createKnowledgeEngine(opts = {}) {
