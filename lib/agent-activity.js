...
async function getProjectDir(workspacePath) {
  ...
  try {
    await stat(computed);
    return computed;
  } catch (err) { tracer.log('warn', `project dir stat failed: ${err.message}`); }
  ...
  try {
    ...
    tracer.log('warn', `projects dir scan failed: ${err.message}`);
  ...
  for (const d of dirs) {
    ...
    } catch (err) { tracer.log('warn', `candidate stat failed: ${err.message}`); }
  }
  ...
  for (const { path: dirPath } of candidates) {
    ...
    if (lastPart && dirName.includes(lastPart)) {
      tracer.log('warn', `Computed path "${encoded}" not found. Using matched fallback: ${dirPath}`);
      return dirPath;
    }
  }
...
async function findLatestSessionFile(projectDir) {
  ...
  try {
    ...
    tracer.log('warn', `session dir readdir failed: ${err.message}`);
    return null;
  }
  ...
  const withStats = await Promise.all(
    jsonlFiles.map(async (f) => {
      ...
      try {
        ...
        tracer.log('warn', `session file stat failed: ${err.message}`);
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        tracer.log('warn', `JSONL line parse failed: ${err.message}`);
      }
    }
    return null;
  } catch (err) {
    tracer.log('warn', `readLastEntry failed: ${err.message}`);
    return null;
  }
}
...
async function parseJsonlTail(filePath, maxBytes = 131072) {
  ...
  try {
    ...
    tracer.log('warn', `parseJsonlTail read failed: ${err.message}`);
    return [];
  }
  ...
  for (const line of safeContent.split('\n')) {
    ...
    try {
      ...
      tracer.log('warn', `JSONL tail line parse failed: ${err.message}`);
    }
  }
  return lines;
}
...
async function getActivityState(workspacePath, readyThresholdMs) {
  ...
  if (!sessionFile) {
    ...
    if (process.env.OPENCLAW_LOG_LEVEL === 'debug') tracer.log('info', `state=${result.state}`);
    return result;
  }
...
async function getSessionInfo(workspacePath, { model } = {}) {
  ...
  if (process.env.OPENCLAW_LOG_LEVEL === 'debug') tracer.log('info', `session: model=${model || 'unknown'} cost=$${info.cost?.estimatedCostUsd?.toFixed(4)}`);
  return info;
}
