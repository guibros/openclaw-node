...
async function getSessionInfo(workspacePath, { model } = {}) {
  ...
  // Extract cost — aggregate from all entries. Session JSONLs nest usage
  // under message.usage on assistant records (top-level line.usage / costUSD
  // are legacy shapes that no longer occur — the old reads summed to zero and
  // every task reported cost:null, found in the D14 rerun 2026-08-03).
  ...
  for (const line of lines) {
    ...
    const u = line.usage ?? line.message?.usage;
    if (u) {
      ...
      if (typeof line.inputTokens === 'number') freshInputTokens += line.inputTokens;
      if (typeof line.outputTokens === 'number') outputTokens += line.outputTokens;
    }
  }
  const inputTokens = freshInputTokens + cacheReadTokens + cacheCreateTokens;
  ...
  if (totalCost === 0 && (inputTokens > 0 || outputTokens > 0)) {
    // Try to detect model from JSONL entries (nested under message.model on
    // assistant records; top-level kept for legacy shapes)
    let detectedModel = model || null;
    if (!detectedModel) {
      for (const line of lines) {
        const lm = line.model ?? line.message?.model;
        if (lm) {
          const m = lm.toLowerCase();
    ...
    // Cache tokens are not full-rate input: reads bill ~0.1x, creation ~1.25x.
    // Agentic sessions are cache-dominated — pricing them at full rate
    // overstated cost ~10x.
    const rates = PRICING[detectedModel] || PRICING.sonnet;
    totalCost = (freshInputTokens / 1_000_000) * rates.input
      + (cacheReadTokens / 1_000_000) * rates.input * 0.1
      + (cacheCreateTokens / 1_000_000) * rates.input * 1.25
