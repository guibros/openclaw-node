...
const tracer = createTracer('agent-activity');
...
async function getProjectDir(workspacePath) {
  ...
  try {
    await stat(computed);
    return computed;
  } catch (err) { console.warn(`[agent-activity] project dir stat failed: ${err.message}`); }
  ...
  try {
    ...
    console.warn(`[agent-activity] projects dir scan failed: ${err.message}`);
  ...
  for (const d of dirs) {
    ...
    } catch (err) { console.warn(`[agent-activity] candidate stat failed: ${err.message}`); }
  }
  ...
  // No name-matched candidate — return computed path (downstream returns null gracefully).
  // Intentionally NOT falling back to most-recent-by-mtime: during concurrent Claude sessions
  // that could return a completely unrelated session's directory.
  return computed;
}
...
async function findLatestSessionFile(projectDir) {
  ...
  try {
    ...
    console.warn(`[agent-activity] session dir readdir failed: ${err.message}`);
    return null;
  }
  ...
  const withStats = await Promise.all(
    jsonlFiles.map(async (f) => {
      ...
      try {
        ...
        console.warn(`[agent-activity] session file stat failed: ${err.message}`);
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        console.warn(`[agent-activity] JSONL line parse failed: ${err.message}`);
      }
    }
    return null;
  } catch (err) {
    console.warn(`[agent-activity] readLastEntry failed: ${err.message}`);
    return null;
  }
}
...
async function parseJsonlTail(filePath, maxBytes = 131072) {
  ...
  try {
    ...
    console.warn(`[agent-activity] parseJsonlTail read failed: ${err.message}`);
    return [];
  }
  ...
  for (const line of safeContent.split('\n')) {
    ...
    try {
      ...
      console.warn(`[agent-activity] JSONL tail line parse failed: ${err.message}`);
    }
  }
  return lines;
}
...
async function getActivityState(workspacePath, readyThresholdMs) {
  ...
  if (!sessionFile) {
    const result = { state: 'starting', timestamp: new Date() };
    if (process.env.OPENCLAW_LOG_LEVEL === 'debug') console.log(`[agent-activity] state=${result.state}`);
    return result;
  }
  ...
  switch (entry.lastType) {
    ...
    case 'tool_use':
    case 'progress':
      result = { state: ageMs > threshold ? 'idle' : 'active', timestamp };
      break;
    ...
    case 'summary':
    case 'result':
      result = { state: ageMs > threshold ? 'idle' : 'ready', timestamp };
      break;

    case 'permission_request':
      result = { state: 'waiting_input', timestamp };
      break;

    case 'error':
      result = { state: 'blocked', timestamp };
      break;
...
async function getSessionInfo(workspacePath, { model } = {}) {
  ...
  // Model-aware pricing (per 1M tokens) — fallback estimates, used only when
  // JSONL doesn't include costUSD. Updated 2026-03 for Claude 4.x pricing.
  const PRICING = {
  ...
  const info = { summary, cost, sessionId };
  if (process.env.OPENCLAW_LOG_LEVEL === 'debug') console.log(`[agent-activity] session: model=${model || 'unknown'} cost=$${info.cost?.estimatedCostUsd?.toFixed(4)}`);
  return info;
}
...
module.exports = {
  toClaudeProjectPath,
  getProjectDir: tracer.wrapAsync('getProjectDir', getProjectDir, { tier: 3, category: 'io' }),
  findLatestSessionFile: tracer.wrapAsync('findLatestSessionFile', findLatestSessionFile, { tier: 3, category: 'io' }),
  readLastEntry: tracer.wrapAsync('readLastEntry', readLastEntry, { tier: 3, category: 'io' }),
  parseJsonlTail: tracer.wrapAsync('parseJsonlTail', parseJsonlTail, { tier: 3, category: 'io' }),
  getActivityState: tracer.wrapAsync('getActivityState', getActivityState, { tier: 3, category: 'io' }),
  getSessionInfo: tracer.wrapAsync('getSessionInfo', getSessionInfo, { tier: 3, category: 'io' }),
