...
async function getProjectDir(workspacePath) {
  ...
  // Return the most recently modified candidate, or computed path as default
  if (candidates.length > 0) {
    // Log warning so we know the encoding drifted
    console.warn(`[agent-activity] Computed path "${encoded}" not found. Using fallback: ${candidates[0].path}`);
 ...
 */
async function getSessionInfo(workspacePath) {
  const projectDir = await getProjectDir(workspacePath);
  const sessionFile = await findLatestSessionFile(projectDir);
  ...
  // Rough estimate if no direct cost data (Sonnet pricing as baseline)
