...
export function querySessionNoteData(db, sessionId) {
  // The sessions table belongs to the session store; an extraction-only DB
  // doesn't have it. The note builders already handle session=null.
  let session = null;
  try {
    session = db.prepare(
...
export function buildSessionFrontmatter(session, entities, opts = {}) {
  ...
  if (entities.length > 0) {
    ...
    // Every entry quoted: bare [[X]] in a YAML flow list parses as nested
    // arrays, which Obsidian/Dataview can't read.
...
/**
 * Backfill session notes for DB sessions that have memory evidence but no
 * note on disk. Runs on the consolidation cadence — session notes used to be
 * born only inside the flush-LLM path, so 193 mention-bearing sessions had 8
 * notes (memory review 2026-07-04 §3A). Newest sessions first, capped per
 * cycle so one run never floods the vault.
 */
export async function backfillSessionNotes(opts = {}) {
  const { db, limit = 20 } = opts;
  if (!db) return { generated: 0, notes: [], remaining: 0 };
  const vaultPath = opts.vaultPath || getVaultPath();
  await ensureVaultStructure(vaultPath);

  const sessionRows = db.prepare(`
    SELECT session_id, MAX(created_at) AS last_seen FROM (
      SELECT session_id, created_at FROM mentions
      UNION ALL
      SELECT session_id, created_at FROM decisions
    ) GROUP BY session_id ORDER BY last_seen DESC
  `).all();

  let existingShortIds = new Set();
  try {
    existingShortIds = new Set(
      readdirSync(join(vaultPath, 'sessions'))
        .map((f) => f.match(/-([0-9a-f]{8})\.md$/i)?.[1])
        .filter(Boolean),
    );
  } catch { /* fresh vault */ }

  const missing = sessionRows.filter((r) => !existingShortIds.has(String(r.session_id).slice(0, 8)));
  const notes = [];
  for (const row of missing.slice(0, limit)) {
    const r = await generateSessionNote({ db, sessionId: row.session_id, vaultPath });
    if (r.generated) notes.push(r.filename);
  }
  return { generated: notes.length, notes, remaining: Math.max(0, missing.length - limit), vaultPath };
}
