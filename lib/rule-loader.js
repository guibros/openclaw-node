  ...
  for (const line of lines) {
    ...
    if (rawVal.startsWith('[')) {
      try {
        ...
        tracer.log('warn', `inline array JSON.parse failed for key "${key}": ${err.message}`);
...
function loadAllRules(rulesDir) {
  if (!fs.existsSync(rulesDir)) {
    tracer.log('info', `Rules dir not found: ${rulesDir}`);
    return [];
  }
  ...
  for (const file of files) {
    try {
      ...
      tracer.log('error', `Skipped ${file}: ${err.message}`);
    }
  }

  tracer.log('info', `Loaded ${rules.length} rules from ${rulesDir}`);
  return rules;
}
...
function matchRules(rules, scopePaths) {
  ...
  tracer.log('info', `matchRules: ${matched.length}/${rules.length} matched for scope`);
  return matched;
}
...
function detectFrameworks(projectDir) {
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    } catch (err) { tracer.log('warn', `package.json parse failed: ${err.message}`); }
  }
  ...
  tracer.log('info', `Detected frameworks: ${detected.join(', ') || 'none'}`);
  return detected;
}
