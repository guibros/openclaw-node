...
function createPlan({
  ...
  tracer.log('info', `Created plan ${plan.plan_id}: ${plan.subtasks.length} subtasks, ${plan.estimated_waves} waves`);
  return plan;
}
...
function assignWaves(subtasks) {
  ...
  if (cycledIds.length > 0) {
    tracer.log('warn', `Cycle detected: subtasks ${cycledIds.join(',')} blocked`);
  }
}
...
function autoRoutePlan(plan, { log } = {}) {
  const logger = log || ((msg) => tracer.log('info', msg));
...
class PlanStore {
  ...
  async markAborted(planId, reason) {
    return this._updateWithCAS(planId, (plan) => {
      ...
      tracer.log('info', `ABORTED: ${planId} — blocked ${blockedCount} pending subtasks`);
      return plan;
    });
  }
  ...
  getNextWaveSubtasks(plan) {
    ...
    if (ready.length > 0) {
      ...
      tracer.log('info', `Wave ${nextWave}: ${ready.length} subtasks ready for dispatch`);
    }
