...
const tracer = createTracer('mesh-plans');
...
function createPlan({
  ...
  console.log(`[plans] Created plan ${plan.plan_id}: ${plan.subtasks.length} subtasks, ${plan.estimated_waves} waves`);
  return plan;
}
...
function assignWaves(subtasks) {
  ...
  for (const [taskId, degree] of inDegree.entries()) {
    if (degree > 0) {
      ...
      if (subtask) {
        subtask.wave = -1; // blocked by cycle
        cycledIds.push(taskId);
      }
    }
  }
  if (cycledIds.length > 0) {
    console.warn(`[plans] Cycle detected: subtasks ${cycledIds.join(',')} blocked`);
  }
}
...
function autoRoutePlan(plan, { log } = {}) {
  const logger = log || ((msg) => console.log(`[plans] ${msg}`));
...
class PlanStore {
  constructor(kv) {
    ...
    tracer.wrapClass(this, [
      'put', 'get', 'delete', 'list', 'findByParentTask',
      'submitForReview', 'approve', 'startExecuting',
      'markCompleted', 'markAborted', 'updateSubtask'
  ...
  async markAborted(planId, reason) {
    return this._updateWithCAS(planId, (plan) => {
      ...
      console.log(`[plans] ABORTED: ${planId} — blocked ${blockedCount} pending subtasks`);
      return plan;
    });
  }
  ...
  getNextWaveSubtasks(plan) {
    ...
    if (ready.length > 0) {
      const nextWave = ready[0].wave;
      console.log(`[plans] Wave ${nextWave}: ${ready.length} subtasks ready for dispatch`);
    }
...
module.exports = {
  createPlan: tracer.wrap('createPlan', createPlan, { tier: 1, category: 'state_transition' }),
  assignWaves: tracer.wrap('assignWaves', assignWaves, { tier: 2, category: 'compute' }),
  routeDelegation: tracer.wrap('routeDelegation', routeDelegation, { tier: 2, category: 'compute' }),
  autoRoutePlan: tracer.wrap('autoRoutePlan', autoRoutePlan, { tier: 2, category: 'compute' }),
  PlanStore,
  PLAN_STATUS,
  SUBTASK_STATUS,
  DELEGATION_MODE,
