...
export function decayWeights(db, opts = {}) {
  ...
  const entities = db.prepare(`
    SELECT id, name, type, canonical_name, first_seen, last_seen, mention_count,
           salience, last_recalled, source_type, source_node, source_event_id
    FROM entities
    WHERE salience > 0
  `).all();
  ...
  const updateSalience = db.prepare(`UPDATE entities SET salience = ? WHERE id = ?`);
  ...
  const doDecay = db.transaction(() => {
    for (const entity of entities) {
      const refDate = entity.last_recalled || entity.last_seen;
      if (!refDate) continue;
  ...
  const decisions = db.prepare(`
    SELECT id, salience, last_recalled, created_at
    FROM decisions
    WHERE salience > 0
  `).all();
  ...
  const updateDecisionSalience = db.prepare(`UPDATE decisions SET salience = ? WHERE id = ?`);

  const doDecayDecisions = db.transaction(() => {
    for (const decision of decisions) {
      const refDate = decision.last_recalled || decision.created_at;
      if (!refDate) continue;
      ...
      if (Math.abs(newSalience - oldSalience) > 0.001) {
        updateDecisionSalience.run(Math.max(newSalience, 0), decision.id);
        decayedDecisions++;
      }
    }
  });
  ...
  return { decayedEntities, decayedDecisions, archivedEntities };
}
...
export async function regenerateSummaries(opts) {
  const { db, client, vaultPath } = opts;

  try {
    const { generateConceptNotes } = await import('./obsidian-summarizer.mjs');
    const result = await generateConceptNotes({ db, client, vaultPath });
    return { regenerated: result.generated || 0 };
  } catch (err) {
    return { regenerated: 0, error: err.message };
  }
}
...
export function evaluatePromotionCandidates(db, opts = {}) {
  ...
  const entityCandidates = db.prepare(`
    SELECT name, type, mention_count, salience
    FROM entities
    WHERE mention_count >= ?
  ...
  const decisionCandidates = db.prepare(`
    SELECT decision, confidence, rationale, created_at
    FROM decisions
    WHERE confidence >= ?
      AND source_type = 'local'
