  ...
  const doDecay = db.transaction(() => {
    for (const entity of entities) {
      ...
      if (clampedSalience < DECAY_DROP_THRESHOLD) {
        ...
        updateSalience.run(clampedSalience, entity.id);
        decayedEntities++;
      }
    }
  });
...
export async function regenerateSummaries(opts) {
  ...
  try {
    ...
    // F-M6's stats pattern: count fallbacks so they're observable.
    const errMsg = err?.message || String(err);
    if (typeof opts.log === 'function') {
      try { opts.log(`[consolidation] regenerateSummaries failed: ${errMsg}`); } catch { /* */ }
    } else {
      // Default: stderr so a misconfigured scheduler still gets the signal.
      // eslint-disable-next-line no-console
      console.error(`[consolidation] regenerateSummaries failed: ${errMsg}`);
    }
    return { regenerated: 0, attempted: 0, skipped: 0, aborted: false, error: errMsg };
  }
}
