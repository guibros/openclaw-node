...
export async function buildGraph(vaultPath) {
  ...
  for (const note of notes) {
    ...
    // Extract wikilinks from the full content (frontmatter related field + body)
    const bodyLinks = extractWikilinks(body);
    for (const target of bodyLinks) {
      const type = resolveEdgeType(frontmatter, target);
      edges.push({ source: note.id, target, type });
    }

    // Also parse the frontmatter `related` field for wikilinks
    if (frontmatter && Array.isArray(frontmatter.related)) {
      for (const rel of frontmatter.related) {
        if (typeof rel === 'string') {
          const relLinks = extractWikilinks(rel);
          for (const target of relLinks) {
            // Avoid duplicate edges already found in body
            const exists = edges.some(
              e => e.source === note.id && e.target === target
            );
            if (!exists) {
              const type = resolveEdgeType(frontmatter, target);
              edges.push({ source: note.id, target, type });
