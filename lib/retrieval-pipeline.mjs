...
/**
 * Filter retrieval results to exclude items linked to private-only entities.
 * When respect_privacy is true, sessions that contain ONLY private entities
 * are filtered out. Sessions with at least one public entity are kept.
 *
 * @param {Array} results — retrieval results with session_id
 * @param {import('better-sqlite3').Database} extractionDb
...
export function filterPrivateResults(results, extractionDb) {
  ...
  try {
    ...
    if (!row || row.cnt === 0) return results;

    // Sessions that have at least one public entity are OK
    ...
    try {
      const rows = extractionDb.prepare(`
        ...
        WHERE e.private = 0
      `).all();
      for (const r of rows) publicSessionIds.add(r.session_id);
    } catch {
      return results;
    }

    // Sessions that have ANY mentions — if none are public, they're private-only
    const mentionedSessionIds = new Set();
    try {
      const rows = extractionDb.prepare(
        'SELECT DISTINCT session_id FROM mentions'
      ).all();
      for (const r of rows) mentionedSessionIds.add(r.session_id);
    } catch {
      return results;
    }

    return results.filter(r => {
      // If session has no mentions at all, keep it (no entity to be private about)
      if (!mentionedSessionIds.has(r.session_id)) return true;
      // If session has at least one public entity, keep it
      return publicSessionIds.has(r.session_id);
    });
  } catch {
    return results;
  }
...
export function createRetrievalPipeline(opts = {}) {
  ...
  async function retrieve(query, queryOpts = {}) {
    ...
    // F-C13: pass respectPrivacy through to all SQL-level entity/theme matchers.
    // Was: post-filter at session granularity (filterPrivateResults), which
    // let private mentions in non-private sessions leak through.
    const chanOpts = { respectPrivacy: privacyFlag };

    // Channel 3: Entity exact match (requires both DBs)
    if (extractionDb && knowledgeDb) {
      resultSets.push(entitySearch(extractionDb, knowledgeDb, query, fetchLimit, chanOpts));
      channelWeights.push(weights.entity);
    }

    // Channel 4: Theme/entity seed (requires both DBs)
    if (extractionDb && knowledgeDb) {
      resultSets.push(themeEntitySearch(extractionDb, knowledgeDb, query, fetchLimit, chanOpts));
      channelWeights.push(weights.theme);
    }

    // Channel 5: Spreading activation (requires all three)
    if (extractionDb && knowledgeDb && graphCache) {
      resultSets.push(activationSearch(extractionDb, knowledgeDb, graphCache, query, 20, chanOpts));
