    ...
    try {
      ...
      const rows = extractionDb.prepare(
        `SELECT DISTINCT d.session_id
         ...
         WHERE decisions_fts MATCH ? AND d.superseded_by IS NULL
         LIMIT ?`
...
export function decisionFtsSearch(extractionDb, knowledgeDb, query, limit = 10, opts = {}) {
  ...
  let sessions;
  try {
    // A superseded decision is history, not guidance: it never seeds recall.
    const privacyClause = opts.respectPrivacy
      ? ' WHERE d.superseded_by IS NULL AND COALESCE(d.private, 1) = 0'
...
export function filterPrivateResults(results, extractionDb) {
  ...
  try {
    ...
    // P3: privacy is SESSION-GRAIN. The former turn-grain branch was dead code on a false
    // premise: result chunks carry a genuine per-turn index (mcp-knowledge stamps turn_index=i),
    // but mentions.turn_index is a per-FLUSH shared marker (the last-turn-of-tail), so the two
    // namespaces never aligned — the branch effectively only matched a flush's final turn and
    // everything else fell through to session-grain anyway (deep review 2026-07-14). Removed so
    // that, if federation privacy is ever switched on (respectPrivacy=true), it can't silently
    // mis-filter. To restore true turn-grain, populate mentions.turn_index with per-entity turn
    // offsets that match the knowledge-chunk namespace, then reintroduce the (session,turn) join.
    const privateSessions = new Set(); // session_id with ANY private mention (fail-safe)
    try {
      const rows = extractionDb.prepare(`
        ...
        WHERE e.private = 1
      `).all();
      for (const r of rows) privateSessions.add(r.session_id);
    } catch (err) {
      reportChannelError('privacy-filter', err);
      return [];  // fail-CLOSED
    }

    // If ANY mention in a chunk's session is private, drop the chunk.
    return results.filter(r => !privateSessions.has(r.session_id));
  } catch (err) {
    reportChannelError('privacy-filter', err);
    return [];  // fail-CLOSED
  }
