...
const tracer = createTracer('kanban-io');
  ...
  while (Date.now() - start < maxWait) {
    try {
      ...
      try {
        ...
        try { fs.rmdirSync(lockDir); } catch { /* Intentional: stale lock cleanup is best-effort */ }
      }
    } catch (err) {
      if (err.code === 'EEXIST') {
...
module.exports = {
  readTasks: tracer.wrap('readTasks', readTasks, { tier: 3, category: 'io' }),
  parseTasks: tracer.wrap('parseTasks', parseTasks, { tier: 3, category: 'io' }),
  updateTaskInPlace: tracer.wrapAsync('updateTaskInPlace', updateTaskInPlace, { tier: 3, category: 'io' }),
  montrealTimestamp,
  isoTimestamp,
  ACTIVE_TASKS_PATH,
};
