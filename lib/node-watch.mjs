...
import { probeGrappeMembers, probeSessionLiveness, gradeClusterQuorum, gradeCoordinator } from './fed-probes.mjs';
...
export const WATCH_TARGETS = [
  ...
  // ── Federation (step 6.3) ────────────────────────────────────────────────
  { id: 'fed.coordinator', family: 'federation', label: 'Grappe coordinator', signal: 'mesh-task-daemon launchd unit loaded',
    async run({ ctx }) {
      if (process.platform !== 'darwin') return U('coordinator launchctl check is darwin-only here');
      const r = await ctx.exec('launchctl', ['list']);
      if (r.code !== 0) return U(`launchctl failed: ${r.stderr.slice(0, 60)}`);
      return mapFed(gradeCoordinator({ loaded: /ai\.openclaw\.mesh-task-daemon/.test(r.stdout) }));
    } },
  { id: 'fed.cluster.quorum', family: 'federation', label: 'NATS bus quorum', signal: ':8222/jsz reachable; cluster quorum if R=3',
    async run({ ctx }) {
      ...
      let clusterSize = 1, membersUp = 1;
      const varz = await ctx.httpGet('http://127.0.0.1:8222/varz', { timeoutMs: 2500 }).catch(() => null);
      // The ctx exposes the parsed body as `.json` (not `.body`) — reading `.body`
      // silently null'd `v`, so this always mis-reported a live R=3 cluster as
      // single-node and could never see a quorum loss. Fixed under step 6.4.
      const v = varz && varz.json;
      const routes = v?.cluster?.urls?.length || 0;
      if (routes > 0) {
        clusterSize = routes + 1;
        // Honest minority detection: self + only the peers actually advertised
        // (capped at configured routes), not a `|| routes` fallback that masks loss.
        membersUp = 1 + Math.min(routes, v?.connect_urls?.length ?? 0);
      }
      return mapFed(gradeClusterQuorum({ jszReachable: true, clusterSize, membersUp }));
    } },
