...
export const WATCH_TARGETS = [
  ...
  { id: 'obs.links', family: 'obsidian', label: 'Vault link integrity', signal: 'no dangling wikilinks / coverage floor',
    run: () => U('no read-only probe yet (vault-check is heavy; wire later)') },
  ...
  { id: 'llm.cloud', family: 'llm-cloud', label: 'Cloud LLM provider', signal: 'configured provider reachable',
    async run({ ctx, config }) {
      // Applicability: only relevant if a cloud provider/key is configured.
      try {
        const env = await ctx.fsp.readFile(ctx.path.join(config.home, 'openclaw.env'), 'utf8');
        const hasKey = /(ANTHROPIC|OPENAI|DEEPSEEK|KIMI|GEMINI|MINIMAX)_API_KEY\s*=\s*\S/.test(env) || /MESH_LLM_PROVIDER\s*=\s*\S/.test(env);
        if (!hasKey) return OFF('no cloud provider/API key configured');
        return U('cloud provider configured but no reachability probe yet');
      } catch { return OFF('openclaw.env not found — cloud LLM not configured'); }
    } },
  ...
  { id: 'ops.calendar', family: 'ops', label: 'Calendar / scheduler', signal: '/api/scheduler/tick reachable',
    run: () => U('no read-only probe yet (POST /api/scheduler/tick would dispatch; needs safe GET)') },
  ...
  { id: 'fabric.deploy_drift', family: 'fabric', label: 'Deploy in sync (repo↔workspace)', signal: 'diff -rq lib empty',
    async run({ ctx, config }) {
      const repoLib = ctx.path.join(process.cwd(), 'lib');
...
export async function runWatch(opts = {}) {
  ...
  const targets = opts.targets || WATCH_TARGETS;
...
export function formatTable(report) {
  ...
  const lines = [
    ...
    `WORKING=${counts.WORKING}  BROKEN=${counts.BROKEN}  OFF=${counts.OFF}  UNKNOWN=${counts.UNKNOWN}`,
    '',
  ];
  let fam = null;
...
export function formatReport(report) {
  ...
  const lines = [
    ...
    `**Tally:** WORKING=${counts.WORKING} BROKEN=${counts.BROKEN} OFF=${counts.OFF} UNKNOWN=${counts.UNKNOWN}`,
    ``,
