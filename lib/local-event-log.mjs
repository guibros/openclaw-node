 ...
 */
export async function createLocalEventLog(nc, nodeId) {
  ...
  async function publishLocal(event) {
    ...
    const subject = `local.${validated.entity_type}.events.${validated.entity_id}.${validated.event_type}`;
    return await js.publish(subject, sc.encode(JSON.stringify(validated)), {
      msgID: validated.idempotency_key,
    });
  }
