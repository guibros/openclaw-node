...
import { gradeCoordinator, gradeClusterQuorum, FED_STATUS } from './fed-probes.mjs';
...
export function buildProbes(ctx) {
  ...
  probes.push({
    ...
    async run() {
      ...
      if (r.code !== 0) return skip(`launchctl unavailable: ${r.stderr.slice(0, 60)}`);
      return mapFedVerdict(
        gradeCoordinator({ loaded: /ai\.openclaw\.mesh-task-daemon/.test(r.stdout) }),
        'mesh-task-daemon loaded ⇒ can coordinate grappes (else worker/standalone)',
      );
    },
  });
  probes.push({
    ...
    async run() {
      ...
      const varz = await ctx.httpGet(`${config.natsMonitorUrl}/varz`, { timeoutMs: 2500 }).catch(() => null);
      const v = varz && varz.json;
      const routes = v?.cluster?.urls?.length || 0;
      if (routes > 0) {
        clusterSize = routes + 1;
        // Honest minority detection: self + only the peers actually advertised in
        // connect_urls (capped at configured routes). Unlike a `|| routes` fallback,
        // an isolated node (0 advertised peers) reads as membersUp=1 ⇒ quorum LOST,
        // not a masked WORKING.
        membersUp = 1 + Math.min(routes, v?.connect_urls?.length ?? 0);
      }
      return mapFedVerdict(
        gradeClusterQuorum({ jszReachable: true, clusterSize, membersUp }),
        'JetStream reachable; R=3 quorum held once cut over (step 1.5)',
      );
    },
  });
