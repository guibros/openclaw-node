...
export function formatMemoryBlock(data = {}) {
  ...
  // Block 7 amendment: surface the degraded-mode warning when LLM analysis
  // fell back to embedding-only. Visible to the receiving LLM so it knows
  // context may be missing implicit references / intent-shaped signals.
  let degradedWarning = '';
  if (analysis && analysis.mode && analysis.mode !== 'llm'
      && analysis.mode !== 'embedding-only' && analysis.mode !== 'noop') {
    // Inline (kept here to avoid cross-file dep; mirrors formatDegradedWarning
    // in memory-injector.mjs). When LLM analysis falls back to embedding-only,
    // the receiving LLM is informed so it knows context may be missing.
    ...
    } else if (reason === 'analysis-wait-timeout' || reason === 'analysis-call-timeout') {
    ...
    degradedWarning = `⚠ degraded mode — ${causeLabel}. Falling back to embedding-only retrieval; results may miss implicit references and intent-shaped context.`;
  }
