...
const LOCAL_MODEL_PROVIDERS = new Set(['ollama']);

function isOpenClawWorkerProvider(name) {
  return typeof name === 'string' && name.length > 0 && !LOCAL_MODEL_PROVIDERS.has(name);
}
