...
export const RECALL_CAPS = {
  ...
  related_sessions: 3,
  themes:           3,
  contradictions:   2,   // always show conflicts even when over cap — operator needs them
};
...
export function queryRelevantConcepts(db, sessionIds, limit = 10, opts = {}) {
  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT e.id, e.name, e.type, e.mention_count, e.last_seen, e.last_recalled,
             AVG(m.salience) as avg_salience
      FROM entities e
      JOIN mentions m ON e.id = m.entity_id
      WHERE m.session_id IN (${placeholders})${privacyClause}
      GROUP BY e.id
      ORDER BY avg_salience DESC, e.mention_count DESC, e.id ASC
      LIMIT ?
...
export function recallScore(item) {
  if (!item) return 0;
  // Recency: time since item was last surfaced. Fall back to last_seen if
  // last_recalled is null (never recalled — use creation/touch time).
  const lastTouchIso = item.last_recalled || item.last_seen;
  let recency = 1;
  if (lastTouchIso) {
    const daysSince = (Date.now() - new Date(lastTouchIso).getTime()) / (1000 * 60 * 60 * 24);
    recency = Math.exp(-Math.max(0, daysSince) / RECENCY_HALF_LIFE_DAYS);
  }
  ...
  const mentionCount = Number.isFinite(item.mention_count)
    ? item.mention_count
    : Number.isFinite(item.mentionCount) ? item.mentionCount : 1;
  const frequency = Math.log1p(mentionCount);
  const salience = Number.isFinite(item.salience) ? item.salience : 0.5;
  const activation = Number.isFinite(item.graph_activation) ? item.graph_activation : 0;
  ...
  const rrf = Number.isFinite(item.rrf_score)
    ? item.rrf_score
  ...
  return recency * frequency * salience * (1 + activation) * (1 + rrf);
}
 ...
 *
 * @param {{ concepts, decisions, snippets, themes?, contradictions? }} data
 * @param {number} [budget] — safety ceiling
 * @returns {{ concepts, decisions, snippets, themes, contradictions, tokenCount, budget, recalled: { entityIds: number[], decisionIds: number[] } }}
 */
export function curateForRecall(data, budget = DEFAULT_TOKEN_BUDGET) {
  ...
  const themes = inhibitWithinGroup(data.themes || [], RECALL_CAPS.themes);
  // Contradictions are always included (high salience, always informative);
  // cap is a soft limit, exceeded if all top items happen to be conflicts.
  const contradictions = inhibitWithinGroup(data.contradictions || [], RECALL_CAPS.contradictions);
  ...
  let tokens = OVERHEAD_TOKENS
    ...
    + themes.reduce((s, t) => s + estimateTokens(t.label || ''), 0)
    + contradictions.reduce((s, c) => s + estimateTokens(c.description || ''), 0);
  ...
  return {
    concepts, decisions, snippets, themes, contradictions,
    tokenCount: tokens,
    budget,
    recalled: {
...
export function createMemoryInjector(opts = {}) {
  ...
  if (llmClient === undefined && process.env.ANALYSIS_MODE !== 'embedding') {
    try {
      ...
      // We synthesize a thunk that resolves the client on first use.
      llmClient = {
        _lazy: true,
  ...
  async function retrieve(prompt, retrieveOpts = {}) {
    ...
    // F-H20 fix: if @memory only:X was passed, filter results to sessions
    // whose top-K mentions include a theme matching X (case-insensitive).
    // Was: themeFilter silently ignored — user typing `@memory only:foo`
    // got full unfiltered retrieval.
