...
/**
 * One peer token → a `nats-route://[user:pass@]host:port` URL. A bare host gets
 * the cluster port. Credentials (item 9): the cluster port is authenticated
 * (user/password — NATS cluster blocks reject token auth), so routes must carry
 * the same credentials or the peer refuses the connection.
 */
function peerToRoute(peer, port = CLUSTER_PORT, { user, pass } = {}) {
  ...
  const hostPort = hasPort ? t : `${t}:${port}`;
  const cred = user && pass ? `${user}:${pass}@` : '';
  return `nats-route://${cred}${hostPort}`;
}

/** Peers → the indented `routes = [ … ]` block body for the cluster config template. */
function renderClusterRoutes(peers, { port = CLUSTER_PORT, indent = '    ', user, pass } = {}) {
  return parsePeers(peers).map((p) => `${indent}${peerToRoute(p, port, { user, pass })}`).join('\n');
}
