...
/** Token cost of sending this tail to the extraction model. */
const tailTokenCount = (tailMessages) =>
  estimateTokens(tailMessages.map((m) => m.content).join('\n'));

/**
 * Tokens that arrived since the last extraction — the only part of the tail a
 * re-extraction actually buys. message_count is already recorded per session,
 * so the new messages are the last (current - recorded) entries of the tail.
 *
 * Returns null when the delta is unknowable, which the policy treats as a
 * reason to extract rather than to guess.
 */
function newTailMaterial(tailMessages, messageCount, prior) {
  if (!prior || typeof prior.message_count !== 'number') return { newTokens: null, saturated: false };
  // Transcript shrank: compaction or rotation rewrote it, so the recorded index
  // no longer refers to the same messages.
  if (messageCount < prior.message_count) return { newTokens: null, saturated: false };
  const newMessages = messageCount - prior.message_count;
  if (newMessages >= tailMessages.length) {
    return { newTokens: tailTokenCount(tailMessages), saturated: true };
  }
  return {
    newTokens: tailTokenCount(tailMessages.slice(tailMessages.length - newMessages)),
    saturated: false,
  };
}
...
const _runFlush = tracer.wrapAsync('runFlush', async function runFlush(jsonlPath, memoryMdPath, opts = {}) {
  ...
  if (useLlm) {
    ...
    const material = newTailMaterial(tailMessages, messageCount, prior);
    const decision = decideExtraction({
      tailTokens: tailTokenCount(tailMessages),
      newTokens: material.newTokens,
      tailSaturated: material.saturated,
      unchanged: prior?.content_hash === contentHash,
      priorExtraction: Boolean(prior),
      deferrable: Boolean(opts.deferrable),
      sessionTokens: opts.sessionTokens ?? null,
      contextWindowTokens: opts.contextWindowTokens ?? null,
      economics: opts.flushEconomics,
    });
    if (!decision.extract) {
      return {
        flushed: false, facts: 0, added: 0, merged: 0, skipped: 1,
        mode: decision.reason === FLUSH_REASONS.UNCHANGED_TAIL ? 'llm-dedup' : 'llm-deferred',
        decision,
        extraction: {
    ...
    try {
      ...
      opts.extractionStore.storeExtractionResult(sessionId, result, undefined, { turnIndex: messageCount - 1 });
