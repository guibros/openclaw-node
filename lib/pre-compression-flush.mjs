...
const _runFlush = tracer.wrapAsync('runFlush', async function runFlush(jsonlPath, memoryMdPath, opts = {}) {
  ...
  if (useLlm) {
    ...
    try {
      ...
      try {
        ...
        try {
          const cov = checkReferentialCoverage({ db: opts.extractionStore.db });
      ...
      try {
        const decisionResult = await generateDecisionNotes({ db: opts.extractionStore.db });
      ...
      try {
        const themeResult = await generateThemeNotes({ db: opts.extractionStore.db });
  ...
  // 4. Write back
  atomicWriteFileSync(memoryMdPath, mergeResult.content);
