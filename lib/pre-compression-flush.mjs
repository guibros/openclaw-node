...
const _extractFacts = tracer.wrap('extractFacts', function extractFacts(tailMessages) {
  ...
  const patterns = [
    ...
    { re: /(?:I'll|I'm going to|I will|let me)\s+(.{10,80})/i, category: 'agent_action', confidence: 70 },
    // Agent findings / observations
    { re: /(?:I found|I noticed|the issue is|the problem is|this is because)\s+(.{10,80})/i, category: 'finding', confidence: 75 },
  ];
  ...
  for (const msg of tailMessages) {
    ...
    for (const { re, category, confidence } of patterns) {
      ...
      for (const match of matches) {
        ...
        facts.push({ fact: factText, category, confidence, speaker: msg.role });
      }
    }
  }
