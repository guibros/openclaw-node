...
const _runFlush = tracer.wrapAsync('runFlush', async function runFlush(jsonlPath, memoryMdPath, opts = {}) {
  ...
  if (useLlm) {
    ...
    try {
      ...
      // resolve to existing ids (aliases) and revisions supersede.
      const knownMemories = typeof opts.extractionStore.findCandidateMemories === 'function'
        ? opts.extractionStore.findCandidateMemories(tailMessages.map((m) => m.content).join('\n'))
        : null;
      const result = await extractStructured(opts.llmClient, tailMessages, { knownMemories });
