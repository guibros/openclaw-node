...
export function mergeFacts(memoryContent, facts, charBudget = 2200) {
  ...
  for (const { fact, category } of facts) {
    // Check against existing entries (strip supersedes comments for clean comparison)
    let bestSim = 0;
    let bestEntry = null;

    for (const entry of entries) {
      const sim = bigramSimilarity(fact, stripSupersedes(entry.text));
      if (sim > bestSim) {
        bestSim = sim;
        bestEntry = entry;
      }
    ...
    if (bestSim > 0.7 && bestEntry) {
      ...
      const oldHash = crypto.createHash('sha256').update(stripSupersedes(bestEntry.text)).digest('hex').slice(0, 8);
      const replacement = `${fact} <!-- supersedes: ${oldHash} -->`;
