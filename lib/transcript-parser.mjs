...
const tracer = createTracer('transcript-parser');
...
// Metadata types to skip: session, model_change, thinking_level_change, queue-operation, custom
const GATEWAY_SKIP_TYPES = new Set([
  'session', 'model_change', 'thinking_level_change',
  'custom', 'queue-operation',
]);
...
registerFormat('openclaw-gateway', {
  ...
  extractMessage(entry) {
    ...
    // Handle toolCall content blocks — render as searchable text
    if (Array.isArray(entry.message?.content)) {
      const toolCalls = entry.message.content.filter(b => b.type === 'toolCall');
      if (toolCalls.length) {
        const toolTexts = toolCalls.map(tc =>
          `[tool_call: ${tc.name || 'unknown'}(${JSON.stringify(tc.arguments || {})})]`
        );
        content = [content, ...toolTexts].filter(Boolean).join('\n');
      }
    }

    // Map toolResult role to standard 'tool' role
    if (role === 'toolResult') {
      role = 'tool';
      if (entry.message.toolName) metadata.toolName = entry.message.toolName;
      if (entry.message.toolCallId) metadata.toolCallId = entry.message.toolCallId;
      if (entry.message.isError !== undefined) metadata.isError = entry.message.isError;
    }

    if (!content) return null;

    // Strip gateway header noise (user/assistant only, not tool results)
    if (role !== 'tool') {
 ...
 */
const _detectFormat = tracer.wrapAsync('detectFormat', async function detectFormat(jsonlPath, sampleLines = 10) {
...
export { _detectFormat as detectFormat };
 ...
 */
const _parseLine = tracer.wrap('parseLine', function parseLine(line, opts = {}) {
...
export { _parseLine as parseLine };
 ...
 */
const _parseJsonlFile = tracer.wrapAsync('parseJsonlFile', async function parseJsonlFile(jsonlPath, opts = {}) {
  ...
  if (!format) {
    format = await _detectFormat(jsonlPath);
  }
...
export { _parseJsonlFile as parseJsonlFile };
 ...
 */
const _estimateFileTokens = tracer.wrapAsync('estimateFileTokens', async function estimateFileTokens(jsonlPath, opts = {}) {
  const { charsPerToken = 4 } = opts;
  const messages = await _parseJsonlFile(jsonlPath, opts);
...
export { _estimateFileTokens as estimateFileTokens };
