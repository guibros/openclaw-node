...
async function checkSqlite() {
  try {
    ...
    // R22 fix (repair 5.6): the 60s health poll needs 'can I read one row',
    // not a full O(database) integrity scan of a growing DB every minute.
    const db = openStore(STATE_DB_PATH, { readonly: true, integrityCheck: false });
    try {
