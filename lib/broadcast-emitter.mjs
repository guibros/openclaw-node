...
export function createBroadcaster(nc, nodeId, opts = {}) {
  ...
  // responding to OUR broadcasts.
  //
  // F-Q103 fix: was a count-based cap (1024 entries, oldest-first eviction).
  // With 1 broadcast/sec that wiped the visible window to ~17 min — but the
  // offer expiry is 60 min by default. Offers responding to legitimate older
  // broadcasts got dropped as 'not_our_broadcast'. Now: Map<event_id, ts>
  // with age-based eviction matched to the broadcast TTL window (24h max
  // per F-N9), checked on each publish + on a periodic sweep.
  const publishedIds = new Map();  // event_id → publishedAtMs
  // Default 25h window — covers max TTL (24h) plus a 1h safety margin so the
  // acceptor doesn't drop legitimate borderline offers. Operator can tune.
  const PUBLISHED_IDS_MAX_AGE_MS = Number(process.env.OPENCLAW_PUBLISHED_IDS_MAX_AGE_MS)
    || (25 * 60 * 60 * 1000);
  // Hard upper bound for memory: even at 1 broadcast/sec for 25h that's 90k
  // entries — too many. Defensive cap so a pathological emitter doesn't OOM.
  const PUBLISHED_IDS_MAX_ENTRIES = 100_000;
  ...
  async function publishBroadcast(themes, entities, intensity, problemClass) {
    ...
    try {
      ...
      stats.emitted++;
      // F-N1 / F-Q103: track for the acceptor's ownBroadcastIds lookup.
      // Age-based eviction matched to offer expiry, with a hard size cap.
      const now = Date.now();
      publishedIds.set(eventId, now);
      // Sweep expired entries (Map insertion order = chronological).
      const cutoff = now - PUBLISHED_IDS_MAX_AGE_MS;
      for (const [id, ts] of publishedIds) {
        if (ts >= cutoff) break;  // first non-expired → all later are also fresh
        publishedIds.delete(id);
      }
      // Defensive cap against pathological emit rate.
      while (publishedIds.size > PUBLISHED_IDS_MAX_ENTRIES) {
        const oldest = publishedIds.keys().next().value;
