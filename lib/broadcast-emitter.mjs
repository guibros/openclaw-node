...
export function computeDedupKey(themes, entities) {
  const combined = [...new Set([
    ...(themes || []).map(t => t.toLowerCase().trim()),
    ...(entities || []).map(e => e.toLowerCase().trim()),
  ])].sort();
...
export function createBroadcaster(nc, nodeId, opts = {}) {
  ...
  async function publishBroadcast(themes, entities, intensity, problemClass) {
    ...
    try {
      ...
      // Eager state update — set BEFORE await would be ideal for F-H8 (race),
      // but we need a successful publish first; this remains best-effort vs
      // perfect-ordering. Race window is small (single-process, NATS round-trip).
  ...
  async function maybeBroadcast(prompt, analysis, broadcastOpts = {}) {
    ...
    // Track theme history for passive-skip logic
    ...
    return await publishBroadcast(themes, entities, intensity, problemClass);
  }
  ...
  return { maybeBroadcast, broadcastFromConsolidation, stop, stats };
}
