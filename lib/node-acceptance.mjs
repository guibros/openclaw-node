...
export function resolveNodeConfig(env = process.env) {
  ...
  return {
    ...
    // The daemon's hard startup dependency (lib/event-schemas.mjs): absent on a
    // node whose install never compiled the workspace package.
    eventSchemasDist: path.join(workspace, 'packages', 'event-schemas', 'dist', 'index.js'),
