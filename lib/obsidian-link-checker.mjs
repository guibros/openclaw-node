...
import { decisionNoteFilename } from './obsidian-decision-notes.mjs';
...
export function checkReferentialCoverage(opts = {}) {
  ...
  try {
    ...
    return {
      ...
      decisions: coverageForDecisions(db, vaultPath),
      themes: coverageForThemes(db, vaultPath, threshold),
    };
  } finally {
    if (ownsDb) db.close();
  }
...
// Repair 2.9: the two new surfaces use the same selection rules as their
// generators (decisions: salience ≥ 0.4 top-30; themes: mention_count ≥ threshold).
function coverageForDecisions(db, vaultPath) {
  const dir = path.join(vaultPath, 'decisions');
  const files = new Set(
    fs.existsSync(dir) ? fs.readdirSync(dir).filter((f) => f.endsWith('.md')) : []
  );
  const eligible = db.prepare(`
    SELECT decision, created_at FROM decisions WHERE salience >= 0.4
    ORDER BY salience DESC, created_at DESC LIMIT 30
  `).all();
  const withNote = eligible.filter((d) => files.has(decisionNoteFilename(d))).length;
  return { eligible: eligible.length, withNote, pct: pct(withNote, eligible.length) };
}

function coverageForThemes(db, vaultPath, threshold) {
  const dir = path.join(vaultPath, 'themes');
  const files = new Set(
    fs.existsSync(dir) ? fs.readdirSync(dir).filter((f) => f.endsWith('.md')).map((f) => f.slice(0, -3)) : []
  );
  const eligible = db.prepare(`SELECT label FROM themes WHERE mention_count >= ?`).all(threshold);
  const withNote = eligible.filter((t) => files.has(slugifyName(t.label))).length;
  return { eligible: eligible.length, withNote, pct: pct(withNote, eligible.length) };
}
