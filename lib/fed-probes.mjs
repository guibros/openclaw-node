...
export function parseLaunchdPrint({ code, stdout = '', stderr = '' } = {}) {
  if (code !== 0) {
    const detail = `${stdout}\n${stderr}`;
    if (/could not find service|service not found/i.test(detail)) {
      return { observable: true, loaded: false, running: false, pid: null, state: null };
    }
    return { observable: false, loaded: false, running: false, pid: null, state: null, error: stderr.trim() || `launchctl exit ${code}` };
  }
  const state = stdout.match(/^\s*state = (.+)$/m)?.[1]?.trim() || 'unknown';
  const rawPid = stdout.match(/^\s*pid = (\d+)$/m)?.[1];
  const pid = rawPid ? Number(rawPid) : null;
  return { observable: true, loaded: true, running: Number.isInteger(pid) && pid > 0, pid, state };
}
...
/** Coordinator process state. Not loaded → OFF (role/standalone); loaded without PID → BROKEN. */
export function gradeCoordinator({ observable = true, loaded, running, pid, state, error }) {
  if (!observable) return { status: FED_STATUS.UNKNOWN, detail: `mesh-task-daemon state unobservable${error ? `: ${error}` : ''}` };
  if (!loaded) return { status: FED_STATUS.OFF, detail: 'mesh-task-daemon not loaded (worker/standalone node)' };
  return running && Number.isInteger(pid) && pid > 0
    ? { status: FED_STATUS.WORKING, detail: `mesh-task-daemon running pid=${pid}` }
    : { status: FED_STATUS.BROKEN, detail: `mesh-task-daemon loaded but not running (state=${state || 'unknown'}, no PID)` };
}
...
async function withBus(fn, timeoutMs = 4000) {
  ...
  try {
    ...
    nc = await connect(natsConnectOpts({ timeout: timeoutMs }));
    return await fn(nc);
  } catch {
    return null;
  } finally {
