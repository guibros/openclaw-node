...
function loadHarnessRules(rulesPath, scope) {
  if (!fs.existsSync(rulesPath)) {
    console.log(`[mesh-harness] No harness rules file at ${rulesPath}`);
    return [];
  }
  try {
    ...
    console.log(`[mesh-harness] Loaded ${filtered.length} harness rules (scope: ${scope})`);
    return filtered;
  } catch (err) {
    ...
    for (const file of allFiles) {
      ...
      if (!inScope) {
        ...
        try {
          ...
          reverted.push(file);
        } catch (revertErr) { console.warn(`[mesh-harness] git checkout revert failed for ${file}: ${revertErr.message}`);
          // Untracked file — remove it
          try {
            fs.unlinkSync(path.join(worktreePath, file));
            reverted.push(file);
          } catch (unlinkErr) { console.warn(`[mesh-harness] unlink failed for ${file}: ${unlinkErr.message}`); }
        }
      }
    }
  } catch (err) {
...
function preCommitSecretScan(worktreePath) {
  ...
  try {
    ...
    try {
      ...
      gitleaksRan = true;
      console.log('[mesh-harness] Using gitleaks for secret scan');
    ...
    if (findings.length === 0 && !gitleaksRan) {
      console.log('[mesh-harness] Gitleaks unavailable, using regex fallback');
...
function runMeshHarness(opts) {
  ...
  if (scopeRules.length > 0 && taskScope && taskScope.length > 0) {
    ...
    if (result.violations.length > 0) {
      ...
      console.log('[mesh-harness] Scope check: PASS');
    }
  }
  ...
  if (blockRules.length > 0) {
    ...
    if (result.blocked) {
      ...
      console.log('[mesh-harness] Block scan: PASS');
    }
  }
  ...
  if (secretRules.length > 0 && worktreePath) {
    ...
    if (result.blocked) {
      ...
      console.log('[mesh-harness] Secret scan: PASS');
    }
  }
  ...
  if (scanRulesForLog.length > 0 && !hasMetric) {
    ...
    if (!anySuspicious) {
      console.log('[mesh-harness] Post-execution scan: PASS');
    }
  }

  console.log(`[mesh-harness] Result: ${pass ? 'PASS' : 'FAIL'} violations=${violations.length} warnings=${warnings.length}`);
