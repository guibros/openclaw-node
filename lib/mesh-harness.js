...
function loadHarnessRules(rulesPath, scope) {
  if (!fs.existsSync(rulesPath)) {
    tracer.log('info', `No harness rules file at ${rulesPath}`);
    return [];
  }
  try {
    ...
    tracer.log('info', `Loaded ${filtered.length} harness rules (scope: ${scope})`);
    return filtered;
  } catch (err) {
    tracer.log('error', `Failed to load ${rulesPath}: ${err.message}`);
    return [];
  }
}
    ...
    for (const file of allFiles) {
      ...
      if (!inScope) {
        ...
        try {
          ...
          reverted.push(file);
        } catch (revertErr) { tracer.log('warn', `git checkout revert failed for ${file}: ${revertErr.message}`);
          // Untracked file — remove it
          try {
            fs.unlinkSync(path.join(worktreePath, file));
            reverted.push(file);
          } catch (unlinkErr) { tracer.log('warn', `unlink failed for ${file}: ${unlinkErr.message}`); }
        }
      }
    }
  } catch (err) {
    tracer.log('error', `Scope check error: ${err.message}`);
  }
...
function preCommitSecretScan(worktreePath) {
  ...
  try {
    ...
    try {
      ...
      gitleaksRan = true;
      tracer.log('info', 'Using gitleaks for secret scan');
    ...
    if (findings.length === 0 && !gitleaksRan) {
      tracer.log('info', 'Gitleaks unavailable, using regex fallback');
    ...
    tracer.log('error', `Pre-commit scan error: ${err.message}`);
  }
...
function runMeshHarness(opts) {
  ...
  if (scopeRules.length > 0 && taskScope && taskScope.length > 0) {
    ...
    if (result.violations.length > 0) {
      ...
      tracer.log('info', 'Scope check: PASS');
    }
  }
  ...
  if (blockRules.length > 0) {
    ...
    if (result.blocked) {
      ...
      tracer.log('info', 'Block scan: PASS');
    }
  }
  ...
  if (secretRules.length > 0 && worktreePath) {
    ...
    if (result.blocked) {
      ...
      tracer.log('info', 'Secret scan: PASS');
    }
  }

  const pass = violations.length === 0;

  // 3.5 Post-execution scan pass log (covers the scan rules path when not suspicious)
  const scanRulesForLog = rulesByEnforcement(rules, 'post_scan');
  if (scanRulesForLog.length > 0 && !hasMetric) {
    const anySuspicious = warnings.some(w => w.rule && scanRulesForLog.some(r => r.id === w.rule));
    if (!anySuspicious) {
      tracer.log('info', 'Post-execution scan: PASS');
    }
  }

  tracer.log('info', `Result: ${pass ? 'PASS' : 'FAIL'} violations=${violations.length} warnings=${warnings.length}`);
...
module.exports = {
  ...
  rulesByEnforcement: tracer.wrap('rulesByEnforcement', rulesByEnforcement, { tier: 2, category: 'compute' }),
  enforceScopeCheck: tracer.wrap('enforceScopeCheck', enforceScopeCheck, { tier: 2, category: 'compute' }),
  postExecutionScan: tracer.wrap('postExecutionScan', postExecutionScan, { tier: 2, category: 'compute' }),
  scanOutputForBlocks: tracer.wrap('scanOutputForBlocks', scanOutputForBlocks, { tier: 2, category: 'compute' }),
  preCommitSecretScan: tracer.wrap('preCommitSecretScan', preCommitSecretScan, { tier: 2, category: 'compute' }),
  postCommitValidate: tracer.wrap('postCommitValidate', postCommitValidate, { tier: 2, category: 'compute' }),
