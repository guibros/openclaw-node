...
class MeshRegistry {
  ...
  async init() {
    ...
    tracer.log('info', `KV bucket initialized: ${KV_BUCKET}`);
    return this;
  }
  ...
  async register(manifest, handlers = {}) {
    ...
    tracer.log('info', `Registered tool: ${manifest.name} (${methods.length} methods) on ${this.nodeId}`);
    ...
    for (const method of methods) {
      ...
      if (handlers[method.name]) {
        ...
        tracer.log('warn', `Method ${method.name} has no handler`);
      }
    }
  ...
  async _processSubscription(sub, handlerKey) {
    for await (const msg of sub) {
      try {
        tracer.log('info', `Request: ${msg.subject}`);
        const request = JSON.parse(sc.decode(msg.data));
        const handler = this.handlers.get(handlerKey);

        if (!handler) {
          tracer.log('warn', `No handler for ${handlerKey}`);
        ...
        tracer.log('warn', `Handler error for ${handlerKey}: ${err.message}`);
        msg.respond(sc.encode(JSON.stringify({
          error: err.message,
    ...
    this.heartbeatTimer = setInterval(async () => {
      for (const [toolName, manifest] of this.manifests) {
        ...
        try {
          ...
          tracer.log('error', `heartbeat failed for ${kvKey}: ${err.message}`);
        }
      }
    }, HEARTBEAT_INTERVAL);
  ...
  async call(nodeId, toolName, methodName, args = {}, timeoutMs = 30000) {
    tracer.log('info', `Call: ${toolName}.${methodName}`);
  ...
  async shutdown() {
    ...
    tracer.log('info', `Shutdown: purging ${this.manifests.size} manifests`);
    ...
    for (const toolName of this.manifests.keys()) {
      ...
      try {
        ...
        tracer.log('warn', `shutdown KV delete failed for ${kvKey}: ${err.message}`);
      }
    }
