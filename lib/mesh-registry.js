...
const tracer = createTracer('mesh-registry');
...
class MeshRegistry {
  constructor(nc, nodeId) {
    ...
    tracer.wrapClass(this, [
      'init', 'register', '_processSubscription', 'startHeartbeat',
      'call', 'listTools', 'shutdown',
    ], { tier: 3, category: 'io' });
  }
  ...
  async init() {
    ...
    console.log(`[mesh-registry] KV bucket initialized: ${KV_BUCKET}`);
    return this;
  }
  ...
  async register(manifest, handlers = {}) {
    ...
    console.log(`[mesh-registry] Registered tool: ${manifest.name} (${methods.length} methods) on ${this.nodeId}`);
    ...
    for (const method of methods) {
      ...
      if (handlers[method.name]) {
        ...
        console.warn(`[mesh-registry] Method ${method.name} has no handler`);
      }
    }
  ...
  async _processSubscription(sub, handlerKey) {
    for await (const msg of sub) {
      try {
        console.log(`[mesh-registry] Request: ${msg.subject}`);
        const request = JSON.parse(sc.decode(msg.data));
        const handler = this.handlers.get(handlerKey);

        if (!handler) {
          console.warn(`[mesh-registry] No handler for ${handlerKey}`);
        ...
        console.warn(`[mesh-registry] Handler error for ${handlerKey}: ${err.message}`);
        msg.respond(sc.encode(JSON.stringify({
          error: err.message,
  ...
  async call(nodeId, toolName, methodName, args = {}, timeoutMs = 30000) {
    console.log(`[mesh-registry] Call: ${toolName}.${methodName}`);
  ...
  async shutdown() {
    ...
    console.log(`[mesh-registry] Shutdown: purging ${this.manifests.size} manifests`);
    ...
    for (const toolName of this.manifests.keys()) {
      ...
      try {
        ...
        console.warn(`[mesh-registry] shutdown KV delete failed for ${kvKey}: ${err.message}`);
      }
    }
