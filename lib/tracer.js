...
const HTTP_MAX_BACKOFF = 5; // stop trying after 5 consecutive failures

function flushHttpBuffer() {
  if (_httpBuffer.length === 0) return;
  if (_httpFailCount >= HTTP_MAX_BACKOFF) {
    _httpBuffer = []; // discard — MC is unreachable
    return;
  }
  ...
  req.on('error', () => { _httpFailCount++; });
  req.on('timeout', () => { req.destroy(); _httpFailCount++; });
  req.end(body);
}
