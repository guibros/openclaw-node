...
export function coerceExtractionResult(raw) {
  ...
  // Known-memory ids (`ref`, `supersedes`): a positive integer, else absent.
  // Models write "#12", "12" or 12; anything else (a name, null) is dropped.
  const toId = (v) => {
  ...
  return {
    entities: filterMap(raw.entities, e => {
      ...
      const ref = toId(e.ref);
      if (ref) out.ref = ref;
      if (Array.isArray(e.aliases)) {
        const aliases = e.aliases.map(a => clip(a, FIELD_CAPS.name)).filter(Boolean).slice(0, MAX_ALIASES_PER_ENTITY);
    ...
    decisions: filterMap(raw.decisions, d => {
      ...
      const supersedes = toId(d.supersedes);
...
export async function extractStructured(client, messages, opts = {}) {
  ...
  const prompt = buildExtractionPrompt(messages, { jsonPrimer: primer, knownMemories: opts.knownMemories });
