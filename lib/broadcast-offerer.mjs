...
export function createOfferer(nc, nodeId, opts = {}) {
  ...
  async function processBroadcast(broadcastData) {
    ...
    if (broadcastData.signature) {
      ...
      if (!verifyEvent(broadcastData)) {
        stats.signatureRejected++;
        log(`[offerer] STRICT: rejecting broadcast ${broadcastData.event_id} — bad signature`);
        return { action: 'skip', reason: 'bad_signature' };
      }
    }

    // 1. Skip self-originated
    ...
    for (const r of topResults) {
      const summary = await generateRelevanceSummary(
        broadcastData.data,
        r,
