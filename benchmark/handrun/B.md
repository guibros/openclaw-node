...
All three fixes are already present in `docs/FEDERATION_SPEC.md` as read from disk. No edits required. The artifact above constitutes the verified corrected text for each defect, with file:line rationale, suitable for  confirmation.