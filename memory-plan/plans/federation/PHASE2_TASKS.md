...
- **T4.1.4** Intake handler: accept a complex task, create the session, assign roles from the manifest → *`mesh.mgmt.*` handler set in the existing bin/mesh-task-daemon.js (DECISIONS D2 — one daemon, §4.6; NOT a second daemon)* · done-when: `mesh.mgmt.create` → session in INTAKE. [code]
