...
| 2 | 2.7 | v2.7 | [ ] | Pipeline mode implemented — the D16 fixed-pass protocol (`PIPELINE_MODE_SPEC.md`, D18) as a fourth `architecture`, with the three circling hang paths (all-nodes barrier, unanimity vote, tier gate) absent from it |
| 2 | 2.8 | v2.8 | [ ] | ⛔ THE UNBLOCK STEP — pipeline mode runs the NEW preregistered five-task benchmark (`audits/step28_pipeline-benchmark/RUN_RULES.md`). Passing the unchanged D3 bar is the SOLE condition for deleting BLOCKED.md (D15/D16) |
...
> **2.7 — Goal:** a grappe session running `architecture: 'pipeline'` reaches a terminal, collectable artifact with no vote, no sub-round, and no gate anywhere in the flow.
> **Needs:** `PIPELINE_MODE_SPEC.md` + D18 (the protocol is specified, not invented during implementation); D16 (the law); `lib/mesh-collab.js` session engine + the `startRecruitedSession` single dispatch seam (3.1 correction); grappe substrate 1.3/1.4; `bin/fed-benchmark.mjs` collector (it must learn this mode's terminal shape, SPEC §6).
> **Feeds:** 2.8 benchmarks this mode and nothing else; the collector accepts its terminal artifact; Blocks 4–5 inherit the deterministic-dataflow law (D16 consequence 4) from this implementation rather than from circling.
> **Verify:** `runtime:` a live pipeline session with **one reviewer deliberately silenced** (never submits, never marked dead) still reaches `status: completed` with a collectable artifact inside the pass budget, and its `degraded` ledger names that reviewer — this is the direct regression for the 3-of-5 run-2 failure · `code:` a test proves `checkConvergence`, reflection `vote`, `current_subround` and `needsGate` are unreachable from the pipeline path, and a unit test covers each of the three T2 per-pass absence behaviours (draft absent ⇒ terminal FAILED; reviews absent ⇒ advance; revision absent ⇒ ship the draft flagged degraded).

> **2.8 — Goal:** pipeline mode passes the new preregistered five-task benchmark, or the premise returns to the operator as a second negative result.
> **Needs:** 2.7 closed; `audits/step28_pipeline-benchmark/RUN_RULES.md` **operator-locked** with its three §0 fields filled (slate, cost bar, frozen SHA) — a DRAFT preregistration is not a lock; `bin/fed-run-driver.mjs` + `bin/fed-benchmark.mjs` (reused, not rewritten); the slate frozen at that SHA; two fresh zero-context judges arranged per pair; the §3 scoring-package repairs verified against the run-2 leak.
> **Feeds:** `BLOCKED.md` deletion on a pass — the only path D15/D16 allow; step 3.5 (Phase-1 gate) and Blocks 4–5 resume behind it; on a fail, the D15 disposition returns to the operator (close the plan, or approve a further redesign).
> **Verify:** `runtime:` ten executions at one frozen SHA; `tally` applies the **unchanged** D3 bar (grappe ≥ max(4, 80% of scored); ties count against); delivery rate and blind-score-among-delivered are reported **separately** (RUN_RULES §5) so a 5/5-delivery-but-loses-quality outcome is not collapsed into the verdict; zero gate observations expected — a non-zero count halts the run as a spec violation (RUN_RULES §1.2) · `visual:` operator confirms the verdict and signs the disposition.
