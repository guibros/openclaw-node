...
| 6 | 6.4 | v6.4 | [A] | Federation test census in CI (nats-binary-gated) + NODE_ACCEPTANCE federation axis — acceptance `federation` axis (FED-L2-COORD/QUORUM), census completeness guard. ⚠️ **CORRECTION 2026-07-16 (D12 §4):** this step's headline "quorum honesty fix" was itself a P0 — it fixed the `.body`→`.json` read but shipped `membersUp = 1 + min(routes, connect_urls.length)`, which counts self twice (`connect_urls` includes the local server), so **quorum loss was structurally undetectable** ("both peers dead" graded WORKING "2/3 up"); its unit test fed the grader synthetic numbers and never touched the parse. Re-grounded on raft `jsz.meta_cluster` + PROVEN by induced outage (audits/probe_honesty_remediation). `runtime:` observed-green-CI half STILL PENDING operator push. See audits/step64_fed_acceptance |
