...
const taskId = `repro-joindispatch-${TAG}-${Date.now()}`;
...
await killAgents();
const kv = await nc.jetstream().views.kv('MESH_COLLAB');
for await (const k of await kv.keys()) if (k.includes('repro-joindispatch')) await kv.purge(k);
const tkv = await nc.jetstream().views.kv('MESH_TASKS');
for await (const k of await tkv.keys()) if (k.includes('repro-joindispatch')) await tkv.purge(k);
await nc.close();
