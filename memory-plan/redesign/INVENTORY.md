...
|-------|------|---------|--------|-------------|
| 0 | 0.1 | v0.1 | [ ] | Symlink runtime lib/ → repo lib/ (close lib drift) |
| 0 | 0.2 | v0.2 | [ ] | Symlink runtime daemon file → repo; restart; confirm current code runs |
...
| Block | Step | Version | Status | Description |
|-------|------|---------|--------|-------------|
| 1 | 1.1 | v1.1 | [ ] | Define memory.* event vocabulary in packages/event-schemas |
| 1 | 1.2 | v1.2 | [ ] | Emit memory.ingested at the ingest boundary |
| 1 | 1.3 | v1.3 | [ ] | Emit memory.extracted at the extract boundary |
...
| Block | Step | Version | Status | Description |
|-------|------|---------|--------|-------------|
| 2 | 2.1 | v2.1 | [ ] | Watcher core: subscribe to the event log, persist one record per op to JSONL |
| 2 | 2.2 | v2.2 | [ ] | Classify each op ok / noop / error (incl. empty-output no-op detection) |
| 2 | 2.3 | v2.3 | [ ] | Store-health probes: row counts, last-write, WAL size, repo↔runtime drift |
...
> **3.2:** tool messages present in state.db for a session that had them.
> **3.3:** `SELECT COUNT(turn_index) FROM mentions WHERE created_at > now-1h` > 0.
...
| Block | Step | Version | Status | Description |
|-------|------|---------|--------|-------------|
| 4 | 4.1 | v4.1 | [ ] | Generate structured MEMORY.md from entity/theme/decision tables (emits memory.synthesized) |
...
> **4.1:** end a session → MEMORY.md updates within seconds with structured sections; memory.synthesized event logged.
...
**40 steps total — 36 local-first (Blocks 0–6) + 4 deferred.** Next step to execute: **0.1**.
