...
| 9 | 9.6 | v9.6 | [ ] | Cross-node integration test for broadcast → offer → accepted round-trip |

> **Step 9.6 scope (operator-added beyond REFERENCE_PLAN per RESUME.md §0 Block 9 frozen decisions).** Create `test/broadcast-cross-node.test.mjs` — deterministic two-node integration test using ephemeral in-process NATS clients (no external broker). Simulates a peer node to validate the full round-trip: emit `context.broadcast` on node A → peer B's offerer receives and produces `context.offer` → A's acceptor scores and selects → A emits `context.accepted` after token-overlap match in subsequent prompt. Tests cover: TTL-expired broadcasts are skipped by offerer, private items don't leak into offers, expires_at on offers is respected by acceptor, accepted artifact_refs flow correctly. Becomes the deterministic equivalent of Block 8's "3 clean cycles" automated gate. Without it, regressions in Block 10+ could silently break the broadcast loop with no guard.
