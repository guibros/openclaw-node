...
| 10 | 10.4 | v10.4 | [x] | Node identity + ed25519 signing infrastructure (`lib/node-identity.mjs`); STRICT verification |

> **Step 10.4 closed.** Created `lib/node-identity.mjs` — per-node ed25519 identity keypair management and event signing module. Exports `getOrCreateIdentity(identityDir?)` (generates ed25519 keypair PEM at `<dir>/identity.key` if absent, loads existing, returns `{ privateKey, publicKey, publicKeyBase64 }`), `signEvent(event, privateKey)` (canonical JSON serialization + ed25519 signature → base64 `signature` + `signer_pubkey` fields), `verifyEvent(event)` (STRICT: present-but-invalid signature → false; unsigned → true for backward compat), `canonicalizeEvent(event)` (deterministic key-sorted JSON excluding signature fields). Added optional `signature` + `signer_pubkey` fields to `EventEnvelopeSchema`. Wired signing into `createLocalEventLog` via `opts.identity`. Added STRICT verification to `broadcast-offerer.mjs` and `broadcast-acceptor.mjs` with `signatureRejected` stats. 12 new tests. 10 positive audit findings, zero corrections, zero Phase 8 patches.
| 10 | 10.5 | v10.5 | [x] | Two-node integration test (`test/federation-2node.test.mjs`) — real NATS, real round-trip |

> **Step 10.5 closed.** Created `test/federation-2node.test.mjs` — two-node integration test with real NATS JetStream. Starts an actual `nats-server` process, spawns 2 isolated openclaw node trees with distinct ed25519 identities, and validates the full federation round-trip (broadcast signed by A → offerer on B verifies signature → B offers → acceptor on A verifies signature → A accepts). 11 `it()` blocks covering: distinct identities, JetStream persistence, full signed round-trip, cross-node signature verification, tampered broadcast rejection (offerer + acceptor), self-originated skip, TTL-expired skip, JetStream consumer read, timing (<5s), context.accepted persistence. Graceful skip when `nats-server` not on PATH. 10 positive audit findings, zero corrections, zero Phase 8 patches.
| 10 | 10.6 | v10.6 | [x] | Three-node council test (`test/federation-3node.test.mjs`) — A broadcasts, B+C offer, A picks |

> **Step 10.6 closed.** Created `test/federation-3node.test.mjs` — three-node council test with real 3-node NATS cluster (R=3 replication). Starts 3 `nats-server` processes with full-mesh cluster routing on ephemeral ports, spawns 3 isolated openclaw node trees (A/B/C) with distinct ed25519 identities. Validates full council pattern: A broadcasts → B and C independently offer → A picks higher-scored → context.accepted. 12 `it()` blocks covering: distinct identities, R=3 stream, broadcast persistence across cluster, full council round-trip with multi-offer selection, dedup_key independence, expired offer handling, tampered broadcast rejection by both offerers, self-originated skip, below-threshold filtering, relevance scoring, context.accepted references, timing (<10s). Graceful skip when `nats-server` not on PATH. 10 positive audit findings, zero corrections, zero Phase 8 patches.
| 10 | 10.7 | v10.7 | [x] | Network resilience: peer-offline + reconnect + dead-peer detection + TTL cleanup |

> **Step 10.7 closed.** Created `lib/federation-resilience.mjs` — peer liveness tracker (`createPeerTracker` with configurable timeout, alive/dead detection, periodic cleanup), expired offer cleanup (`cleanupExpiredOffers` for TTL-based pending offer purging), and NATS reconnect constants (`NATS_RECONNECT_OPTS` with infinite retries, 2s wait, 1s jitter). Modified `lib/broadcast-offerer.mjs` — integrated peer tracker to log dead-peer return events (`deadPeerLogged` stat). Modified `lib/broadcast-acceptor.mjs` — integrated peer tracker for dead-peer filtering in `getTopOffer()` (`deadPeerFiltered` stat), periodic cleanup timer for expired offers (`expiredCleaned` stat), imported `cleanupExpiredOffers`. Modified `workspace-bin/memory-daemon.mjs` — merged `NATS_RECONNECT_OPTS` into NATS connect call for auto-reconnect with backoff, added `natsConn.status()` async iterator for reconnect/disconnect/error event logging. 15 new tests. 10 positive audit findings, zero corrections, zero Phase 8 patches.
| 10 | 10.8 | v10.8 | [x] | `docs/MULTI_NODE_DEPLOY.md` — soup-to-nuts deployment guide for real-hardware council |

> **Step 10.8 closed.** Created `docs/MULTI_NODE_DEPLOY.md` — comprehensive 654-line deployment guide for standing up a 3-node OpenClaw council on real hardware. Covers: prerequisites (Node.js ≥18, nats-server ≥2.6), architecture diagram, single-machine dev setup (NATS cluster, spawn-node, identity keypairs, shared stream verification, daemon startup, end-to-end round-trip), multi-machine deployment (per-machine prep, NATS cluster config, firewall rules, Tailscale, systemd services), verification (cluster health, R=3 stream, identity, integration tests), environment variables reference (17 vars across 5 categories), troubleshooting (7 failure modes), and rollback (node removal, single-node revert, cleanup). Documentation-only step, zero functional code changes, zero new tests. 10 positive audit findings, zero corrections, zero Phase 8 patches.
| 10 | 10.9 | v10.9 | [x] | Dogfood harness (`bin/dogfood-council.mjs` + `docs/DOGFOOD_PROTOCOL.md`) |

> **Step 10.9 closed.** Created `bin/dogfood-council.mjs` — dogfood harness CLI + library for 3-node federation council observation. Exports `createDogfoodHarness` (spawns nodes via spawn-node, connects to NATS, starts metric collection), `createMetricCollector` (subscribes to `context.broadcast/offer/accepted` + `mesh.health.alerts`, classifies events into 7 metric types, writes JSONL), `aggregateFromLines`/`aggregateMetrics` (computes broadcast count, offer count, accepted count, offer-to-acceptance ratio, avg round-trip ms via event_id correlation, signature failures, dead-peer events, per-node breakdown, duration), `formatStatsReport` (markdown table output). CLI supports `--node-ids`, `--nats-url`, `--metrics-path`, `--duration`, `--stats` flags with SIGINT graceful shutdown. Created `docs/DOGFOOD_PROTOCOL.md` — comprehensive dogfood protocol documentation with quick start, metrics reference table, green/yellow/red health thresholds, result interpretation guide, troubleshooting, and rollback. 22 new tests. 10 positive audit findings, 1 negative (test count underestimate), zero Phase 8 patches. **Block 10 complete (9/9).**
